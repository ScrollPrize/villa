...
_ENUMS = {
    ...
    "track_radius_robust_loss": ["power", "cauchy"],
}
...
_NULL_TYPES = {
    ...
    "track_radius_cauchy_scale": "number",
    "psi_volume_path": "string",
}
...
class Config:
    def __init__(self, overrides=None):
        ...
        # Robust loss function applied to per-point track radius deviations.
        # "power" (default) uses the existing |hinged|^p norm.
        # "cauchy" applies the Cauchy (Lorentzian) M-estimator, which
        # suppresses outlier gradients more aggressively than power norms,
        # preventing straddler tracks from pulling the spiral between windings.
        self.track_radius_robust_loss = "power"
        # Scale parameter for the Cauchy loss.  None = auto-compute as
        # dr_per_winding / 2 (half the winding spacing), which puts a
        # half-winding error firmly in the outlier-suppression regime.
        self.track_radius_cauchy_scale = None
        ...
        # C2c: Winding consistency loss — penalises disagreement between
        # spiral winding assignment and the psi field from C0.
        # Disabled by default (weight 0.0); set > 0 to activate.
        self.loss_weight_winding_consistency = 0.0
        # Path to the exported winding field zarr (from export_winding_field.py).
        # None = not available; the winding consistency loss is silently skipped.
        self.psi_volume_path = None
        # Huber delta for winding consistency loss (in winding units).
        # 0.3 allows sub-winding fluctuations without penalty.
        self.winding_consistency_huber_delta = 0.3
