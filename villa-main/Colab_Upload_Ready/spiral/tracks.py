...
def _grouped_same_radius_loss(
    ...
    robust_mode = cfg.get('track_radius_robust_loss', 'power')
    if robust_mode == 'cauchy':
        from spiral_helpers import _cauchy_abs
        cauchy_scale = cfg.get('track_radius_cauchy_scale')
        if cauchy_scale is None:
            # Default: half the winding spacing in canonical radius units.
            # This puts a half-winding error firmly in the log regime where
            # its gradient is suppressed by ~80%.
            cauchy_scale = float(dr_per_winding.detach()) / 2.0
        # Guard: cauchy_scale must be > 0 to avoid division by zero.
        cauchy_scale = max(cauchy_scale, 1e-6)
        per_point = _cauchy_abs(hinged, cauchy_scale)
    else:
        # Original power norm path (default, backward compatible).
    ...
    # For the power norm, apply the inverse-power normalization.
    # For Cauchy, the loss is already in natural units — skip this step.
    if robust_mode != 'cauchy':
        within_p = cfg['track_radius_within_norm_p']
        if within_p != 1.0:
