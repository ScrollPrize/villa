...
def export_winding_field(
    ...
    res = resolution or int(src_attrs.get("scaledown") or src_attrs.get("resolution_factor") or 4)

    # Write output — match the pattern used in labels_to_winding_volume.py:
    # zarr.open with shape/dtype creates a root Array (not a Group).
    # We wrap it in a Group so we can store the data under a named dataset
    # and also set group-level attrs.
