...
| `ink_thumb.py` | `top-prediction-small.webm` | opens on flatten's exact framing, then pans right (no zoom) as the scan starts: a close-by output sheet sits just right/behind the red one like a real projection screen; the camera dwells a long beat on the projection — a tiny glowing inference kernel sweeps the intact red sheet, its reading beamed across the short gap to a synced write head rastering the ink prediction line by line — then one slow direct zoom dives to a letter-by-letter cruise on a line ~2/3 down (the kernel's screen speed stays steady — the zoom absorbs the slow-down), and one last zoom pulls straight out to the completed reconstruction: the text finishes filling in as it lands, the rig untilts to a flat dead-on view, the red input fades away, and the finished prediction (a real PHerc. Paris 4 prediction) holds padded for the last second (300 frames) |
