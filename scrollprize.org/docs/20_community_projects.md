...
- [vesuvius-repro](https://github.com/TAUIL-Abd-Elilah/vesuvius-repro): Checks whether the published surface predictions can be regenerated from public inputs, for any scroll. All 36 scrolls with a published m7 surface prediction were checked, and all 36 reproduce at Dice 0.9983–1.0000, with the residual disagreement confined to voxels within 0.01 of the threshold. They do not all reproduce under the same settings, though: 35 need test-time augmentation off and PHerc. Paris 4 needs it on (0.8907 without, 0.9999 with), and nothing in the artifacts records which was used. Also catalogues which prediction sits on which CT level. By TAUIL Abd Elillah
