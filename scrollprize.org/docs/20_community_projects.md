...
- [scroll-audit](https://github.com/bkalita-git/scroll-audit) by Bipul Kalita. Extends catalog auditing down to **segments**: validates each published `tifxyz` surface's `meta.json` (bbox, `scale`, grid dimensions, parent references) alongside the volume-level catalog checks, and with `--deep-bbox` recomputes a segment's true bbox from its published `x/y/z.tif` grids over valid (`!= -1`) points. Complements [scroll-data-audit](https://github.com/Bullo27/scroll-data-audit), which covers the catalog/Zarr-array side. Found a stale volume shape on Scroll 1 ([#1617](https://github.com/ScrollPrize/villa/issues/1617)) and 28 PHercParis4 w-series segments whose bbox lower corner is the tifxyz `-1` sentinel, with corrected per-segment values ([#1618](https://github.com/ScrollPrize/villa/issues/1618)); also probed all 3,963 data origins declared in `metadata.json` (0 dead).
