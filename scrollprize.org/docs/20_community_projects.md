...
- [absolute-winding](https://github.com/tonclap/absolute-winding) by Grigoriy Dyachkov. Measures the one quantity relative winding fields leave undetermined: the origin. Laminae counted outward from the umbilicus in the 2 µm recto surface prediction track true winding ~1:1 across windings 10–129 (slope 0.978–1.070, r ≥ 0.978) but sit a constant below it, giving `absolute = counted + 6 (+1/−2)` for PHerc. Paris 4 over z ≈ 3100–15700, measured against the 28 published segments whose names are absolute winding intervals, `w010-027` … `w128-129`. That numbering is not independent evidence: the spiral fit takes absolute winding annotations as an input and no other input carries an absolute number, so the segment names are the 59 hand-placed `abs_winding.json` labels propagated — both figures (−5.5 and −7) are reported as one truth and its spread, not as agreement. Carries its own applicability gate: the slope needs no absolute labels, and at the top of the `z3000_18000` fit domain it drops to 0.465 (r 0.574) and refuses the slice, where a second estimator diverges with it. Every hit rate is reported against a shuffled-label null with the constant reselected inside each permutation (0.277 measured, 0.064 null). Calibrates the field, not individual points: dispersion grows outward (IQR 5 → 20) and points are scored against intervals 2 to 18 windings wide, on which generous test one lands inside 20–32% of the time — the per-point absolute number is not recovered. Also documents the cross-scan mesh trap that makes the cheap `45.532um.tifxyz` grid score at its own random baseline.
