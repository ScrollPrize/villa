...
- [PHerc1218 spiral-fit sweep — fitted surfaces](https://www.kaggle.com/datasets/pscamillo/pherc1218-spiral-fit-sweep-surfaces) by pscamillo. At-scale fitted surfaces for PHerc1218 (Grand Prize-eligible, zero human segments): 21 windows across both anchored bands, 19 validated against pre-registered criteria, produced with the unmodified official `fit_spiral` off the machine-generated input pack by Iyán Dopico ([vesuvius-sheet-tools](https://github.com/IyanDopico/vesuvius-sheet-tools)). Per-window table, criteria and portable QA tools in [results/pherc1218_sweep](https://github.com/IyanDopico/vesuvius-sheet-tools/tree/main/results/pherc1218_sweep). The replication failed the pack's published gate and was reported rather than tuned away, leading the pack author to a runner bug that was silently degrading constraint sampling; the sweep also surfaced a zero-patch fit path in `fit_spiral.py` ([#1237](https://github.com/ScrollPrize/villa/issues/1237)).
