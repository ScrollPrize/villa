...
- [ink-constraints](https://github.com/Nicodol/ink-constraints) by Nicolas Dolegieviez. Turns the published ink detections into same-winding point collections that `fit_spiral` consumes directly, as one more `pcl_json_paths` entry with no villa changes. Ordered chains are extracted from the ink rasters and retro-projected through the band meshes into the dataset frame, with locality, fold and mesh sheet-switch guards, and every point within 10 voxels of a held-out evaluation patch is dropped. 3,287 chains and 15,065 points across all 28 published PHerc. Paris 4 bands, each chain paired with a shape-matched placebo twin on ink-free sites of the same mesh, so the contribution of the ink itself can be separated from generic re-import of band geometry. Whether this improves a real fit is being measured by a three-arm campaign preregistered and sealed before any comparison run (tag `prereg-a1-sealed`); the verdict will be published whatever it turns out to be. MIT, 116 tests.
