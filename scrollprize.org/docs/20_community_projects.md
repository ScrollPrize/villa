...
- [Spec 1.5: MedNeXt-L + SkeletonRecall surface predictor](https://github.com/ciscoriordan/mednext-vs-umamba-scroll) by Francisco Riordan. On a 7-cube held-out Scroll-1 benchmark scored with the official Kaggle Vesuvius Surface Detection metric, it scores 0.4397 vs the d058 production model's 0.3996 (+10.0%), winning on all 7 cubes; best deployed as a voxel-wise `max(d058, MN)` ensemble (0.4437). Targets the compressed/curved-region failure mode of [#191](https://github.com/ScrollPrize/villa/issues/191). Drop-in compatible with d058's inference contract (OME-Zarr or Zarr). Trainer: [#975](https://github.com/ScrollPrize/villa/pull/975). Weights: [HuggingFace](https://huggingface.co/ciscoriordan/mednext-l-scroll-surface).
