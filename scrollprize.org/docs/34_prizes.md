...
There are many items here that could take quite a while to create, but there are few tasks too small to be considered for a prize -- take a look towards the bottom of this list for some examples of shorter projects intended to take a few hours or a few days at most. 
...
**Shorter Projects**
- Reworks of documentation or site pages
- Testing / Reviews of normalization on scroll data and impact on model performance
- Testing / Reviews of loss functions on scroll data and impact on model performance
- Quality metrics that correlate well with good predicitons on fibers or surface models, and a pipeline script for running them on tif stacks or zarrs