...
the code path, drop the tests that exist only for it, and delete its paragraph from the
README. (Checkpoint configurations now load tolerantly, dropping keys the schema no
longer has with a NOTE, so no retired-key registry is needed.)
...
## Status on `spiral-gap-pin-simplified` (2026-09-24)

The pinned fit was re-applied on top of the spiral simplification (#1871), with:

- every key in the Retire table removed (the demotion re-check is the constant
  `fit_spiral.PIN_DEMOTE_RECHECK_INTERVAL = 1000`; the strain loss always detaches its
...
Both target paths (`model_pin_targets_integer`, still off by default, and the trainable
fractional `T`) are kept until the experiment below decides between them.
