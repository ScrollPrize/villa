...
def _piecewise_triton(query, knot_x, knot_y, knot_counts):
    ...
    result = pinned_affine_map(query, knot_x, knot_x, knot_y, scale, knot_counts)
    # This radius deformation uses unit slope also below its fixed origin.
    first_x, first_y = knot_x[:, 0], knot_y[:, 0]
    if query.ndim == 2:
