...
class LivePatchRelinkTests(unittest.TestCase):
    ...
    def test_loaded_collections_carry_their_catalog_id_whatever_the_role(self):
        # Cross-patch views of absolute, drawn, and legacy collections are
        # matched back to the catalog through resident_collection_id when a
        # live patch relinks them; only editable roles used to carry it.
        for role, path in (('absolute', '/inputs/abs_winding.json'),
                           ('drawn_control_points', '/inputs/drawn.json'),
                           (None, '/inputs/legacy.json'),
                           ('same_winding', '/inputs/same_winding.json')):
            pcl = {'id': 3, 'points': {}}
            fit_spiral.stamp_loaded_pcl_metadata(pcl, path, role, 42)
            self.assertEqual(pcl['metadata']['resident_collection_id'], 42, role)
            self.assertEqual(pcl['metadata']['input_role'],
                             role or 'legacy')
        self.assertEqual(pcl['metadata']['logical_input_kind'], 'same_winding')
        self.assertEqual(pcl['metadata']['logical_input_id'], '3')

    def test_relinked_legacy_collection_does_not_duplicate_its_cross_patch_view(self):
        # A non-editable collection that already has a cross-patch view gains
        # another attachment: the old view is replaced, not kept alongside.
        cid = 5
        pcl = self._regular_pcl(cid, [
            [50.0, 10.0, 10.0], [50.0, 20.0, 20.0],
            [50.0, 510.0, 510.0], [50.0, 520.0, 520.0]])
        del pcl['metadata']['resident_collection_id']
        fit_spiral.stamp_loaded_pcl_metadata(
            pcl, pcl['source_file'], None, cid)
