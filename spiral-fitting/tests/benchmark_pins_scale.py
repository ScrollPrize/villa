...
from transforms import SpiralAndTransform, ray_specialized_spiral_to_scroll
...
def main():
    ...
    parser.add_argument('--per-ray', type=int, default=32)
    ...
    torch.manual_seed(1)
    # Loss-like sample sets: spiral-space points across the windings, scroll
    # space points near the sheets, and ray bundles sharing (theta, z).
    n = args.points
    ...
    theta_r = torch.rand([args.rays], device=device) * TWO_PI
    z_r = torch.rand([args.rays], device=device) * (z1 - z0) + z0
    ids = torch.arange(args.rays, device=device).repeat_interleave(args.per_ray)
    rad_r = dr * (torch.rand([args.rays * args.per_ray], device=device) * 110 + 15)
    report = {'device': device, 'torch': torch.__version__, 'registry_pins': model.pin_registry.num_pins,
              ...
              'rebin_interval': args.rebin_interval, 'points': n, 'rays': args.rays,
              'per_ray': args.per_ray, 'iterations': args.iterations, 'warmup': 2}
    ...
    def run(pinned, part):
        ...
        if part == 'inverse':
            ...
            out = ray_specialized_spiral_to_scroll(tr, rad_r, theta_r, z_r, ids, theta_r.sin(), theta_r.cos())
        elif part == 'build':
            return
    ...
    for part in ('build', 'inverse', 'forward', 'rays'):
