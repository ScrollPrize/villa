...
def test_strain_gradcheck_double():
    torch.manual_seed(0)
    rays, windings = 5, 8
    dr = torch.tensor(16.0, dtype=torch.float64)
    ...
    # The target (built from the target column, theta and dr) is detached
    # inside the loss, so only the paths through s_free are checked: the
    # intermediate radius and the free gaps.
    r = r.requires_grad_(True)

    def fn(r, gaps):
        pins_leaf = torch.stack([z, theta, r, target], -1)
        return get_pin_strain_loss(pins_leaf, _FreeTableStage(gaps, dr), dr, margin=0.01)[0]

    assert torch.autograd.gradcheck(fn, (r, gaps), atol=1e-6, rtol=1e-5)
