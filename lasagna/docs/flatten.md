...
1. `flatten_solver` omitted, `"torch"`, or `"inverse"`
   ...
   - Optimizes `flatten_map_ms` as a regular output-grid map:
...
2. `flatten_solver: "forward"`
   - New source-UV path.
   - Optimizes `flatten_map_ms` as a source-grid UV field:
...
The optimizer is still plain Adam over `flatten_map_ms`; no sparse matrix solve
