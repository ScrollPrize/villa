...
def decode_dir_angle_torch(dir0, dir1):
    """Torch equivalent of :func:`decode_dir_angle`."""

    import torch

    cos2t = 2.0 * dir0 - 1.0
    sin2t = cos2t - torch.sqrt(torch.as_tensor(2.0, dtype=dir0.dtype, device=dir0.device)) * (
        2.0 * dir1 - 1.0
    )
    return torch.atan2(sin2t, cos2t) * 0.5
...
def estimate_normal_torch(
    ...
    """Torch equivalent of :func:`estimate_normal`.

    The formula is intentionally kept in lockstep with the NumPy helper above
    so CUDA callers can use Lasagna's established three-plane ambiguous normal
    reconstruction without copying candidate fields back to CPU.
    """

    import torch

    theta_z = decode_dir_angle_torch(dir0_z, dir1_z)
    theta_y = decode_dir_angle_torch(dir0_y, dir1_y)
    theta_x = decode_dir_angle_torch(dir0_x, dir1_x)

    sz, cz = torch.sin(theta_z), torch.cos(theta_z)
    sy, cy = torch.sin(theta_y), torch.cos(theta_y)
    sx, cx = torch.sin(theta_x), torch.cos(theta_x)
    ...
    dot2 = n1_x * n2_x + n1_y * n2_y + n1_z * n2_z
    sign2 = torch.where(dot2 >= 0.0, torch.ones_like(dot2), -torch.ones_like(dot2))
    n2_x = n2_x * sign2
    n2_y = n2_y * sign2
    n2_z = n2_z * sign2

    dot3 = n1_x * n3_x + n1_y * n3_y + n1_z * n3_z
    sign3 = torch.where(dot3 >= 0.0, torch.ones_like(dot3), -torch.ones_like(dot3))
    n3_x = n3_x * sign3
    n3_y = n3_y * sign3
    n3_z = n3_z * sign3
    ...
    est_y = s1 * n1_y + s2_s * n2_y + s3_s * n3_y
    est_z = s1 * n1_z + s2_s * n2_z + s3_s * n3_z
    norm_e = torch.sqrt(est_x**2 + est_y**2 + est_z**2) + eps
    est_x = est_x / norm_e
    est_y = est_y / norm_e
    est_z = est_z / norm_e

    wz2 = torch.sqrt(est_x**2 + est_y**2 + eps)
    wy2 = torch.sqrt(est_x**2 + est_z**2 + eps)
    wx2 = torch.sqrt(est_y**2 + est_z**2 + eps)
    ...
    dot2r = rn1_x * rn2_x + rn1_y * rn2_y + rn1_z * rn2_z
    s2r = torch.where(dot2r >= 0.0, torch.ones_like(dot2r), -torch.ones_like(dot2r))
    rn2_x = rn2_x * s2r
    rn2_y = rn2_y * s2r
    rn2_z = rn2_z * s2r

    dot3r = rn1_x * rn3_x + rn1_y * rn3_y + rn1_z * rn3_z
    s3r = torch.where(dot3r >= 0.0, torch.ones_like(dot3r), -torch.ones_like(dot3r))
    rn3_x = rn3_x * s3r
    rn3_y = rn3_y * s3r
    rn3_z = rn3_z * s3r
    ...
    ny_f = rn1_y + rn2_y + rn3_y
    nz_f = rn1_z + rn2_z + rn3_z
    norm_f = torch.sqrt(nx_f**2 + ny_f**2 + nz_f**2) + eps
    nx_n = nx_f / norm_f
    ny_n = ny_f / norm_f
    nz_n = nz_f / norm_f

    w_z = torch.sqrt(nx_n * nx_n + ny_n * ny_n + eps)
    w_y = torch.sqrt(nx_n * nx_n + nz_n * nz_n + eps)
    w_x = torch.sqrt(ny_n * ny_n + nz_n * nz_n + eps)
...
__all__ = [
    "decode_dir_angle",
    "decode_dir_angle_torch",
    "encode_normal_nxny_u8",
    "estimate_normal",
    "estimate_normal_torch",
]
