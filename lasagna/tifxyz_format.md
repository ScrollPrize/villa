...
The grid is a *regular* 2D lattice in index space; `meta.json.scale` gives its sampling density in grid cells per voxel.
...
`scale = [sx, sy]` is the sampling density of the grid: grid cells per voxel along each axis. Adjacent grid vertices are `1/sx` (resp. `1/sy`) voxels apart, and a grid of `cols x rows` cells spans `cols/sx x rows/sy` voxels (`QuadSurface::size()`). The tracer writes `0.05` for its 20-voxel step.
