...
def pred_dt_loss_map(*, res: fit_model.FitResult3D, sqrt: bool = True) -> tuple[torch.Tensor, torch.Tensor]:
	...
	When sqrt=True (default), applies sqrt for a sublinear penalty.
	When sqrt=False, uses linear distance values.
	...
	sampled_raw = grid_sample_3d_u8_diff(vol, xyz, offset, inv_scale)  # (1, D, Hm, Wm) float32
	lm = sampled_raw.clamp(min=0.0)
	if sqrt:
		lm = lm.clamp(min=0.01).sqrt()
	lm = lm.permute(1, 0, 2, 3)  # (D, 1, Hm, Wm)
	mask = res.mask_lr
	return lm, mask


def pred_dt_loss(*, res: fit_model.FitResult3D, sqrt: bool = True) -> tuple[torch.Tensor, tuple[torch.Tensor, ...], tuple[torch.Tensor, ...]]:
	lm, mask = pred_dt_loss_map(res=res, sqrt=sqrt)
	wsum = mask.sum()
	if float(wsum) > 0.0:
