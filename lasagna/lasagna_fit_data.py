...
class DataVolume(nn.Module):
	"""Residual pyramid representation of normal + winding + validity 3D volumes."""

	def __init__(self, normal_pyr: nn.ParameterList, winding_pyr: nn.ParameterList,
				 validity_pyr: nn.ParameterList) -> None:
		super().__init__()
	...
	def from_initial(
		...
		return cls(normal_pyr, winding_pyr, validity_pyr)
...
def _scatter_mesh_to_volume(
	...
	Returns (normal_sum, winding_sum, validity_sum, count) each at vol_shape.
	"""
...
def fit_data_to_model(
	*,
	...
	with torch.no_grad():
		normal_sum, winding_sum, validity_sum, count = _scatter_mesh_to_volume(
			xyz_lr, mask_lr, mesh_normals, winding_target.expand(D, 1, Hm, Wm),
			validity_target_mesh,
	...
	data_vol = DataVolume.from_initial(normal_sum, winding_sum, validity_sum, count, device=dev)
	...
	param_groups = []
	for i, (np_, wp_, vp_) in enumerate(zip(
			data_vol.normal_pyr, data_vol.winding_pyr, data_vol.validity_pyr)):
		lr_level = lr * (lr_exponent ** (n_levels - 1 - i))
	...
	del data, normal_sum, winding_sum, validity_sum, count
	...
	if normal_vis_dir is not None:
		...
		_write_normal_vis_slices(vis_dir, final_normal, final_winding,
							validity=final_validity, labels=labels_vol, ds=vis_ds)
	...
	if output_path is not None:
		...
		print(f"[fit_data] wrote normal {final_normal.shape}, winding {final_winding.shape}, "
			  f"validity {final_validity.shape}", flush=True)
