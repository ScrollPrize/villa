...
*/
#include "mean_normalized_solver.h"
...
__global__ void update_nodes_kernel_f_star_step(Node* d_graph, size_t* d_valid_indices, float o, float spring_constant, float step_sigma, int num_valid_nodes, int estimated_windings, int i_round) {
    ...
    for (int j = 0; j < node.num_edges; ++j) {
        ...
        float abs_dist_factor = fabsf(fmaxf(fabsf(dist_factor), 0.001f) - 0.50f);
...
__global__ void update_f_star_kernel_step(Node* d_graph, size_t* d_valid_indices, int num_valid_nodes, float median_f_star, int teflon_winding_nr, bool blow_away) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= num_valid_nodes) return;
...
std::vector<Node> run_solver_f_star_normalized(std::vector<Node>& graph, int num_iterations, std::vector<size_t>& valid_indices, Edge** h_all_edges, float** h_all_sides, int i_round, float o, float spring_constant, float step_sigma, int teflon_winding_nr, bool visualize, bool adjust_median, bool blow_away) {
    cudaDeviceReset();
    ...
    std::cout << "Adjust median: " << adjust_median << std::endl;
    ...
    for (int iter = 1; iter < num_iterations; iter++) {
        ...
        update_nodes_kernel_f_star_step<<<blocksPerGrid, threadsPerBlock>>>(d_graph, d_valid_indices, o_current, spring_constant, step_sigma, num_valid_nodes, 200, i_round);
        ...
        update_f_star_kernel_step<<<blocksPerGrid, threadsPerBlock>>>(d_graph, d_valid_indices, num_valid_nodes, median_f_star, teflon_winding_nr, blow_away);
        median_f_star = 0.0f;
