...
def train_one_epoch(
    ...
    for data_iter_step, (samples, targets) in enumerate(
        ...
        wandb_log_steps = getattr(args, "wandb_log_steps", 0)
        if wandb_log_steps and wandb and wandb.run and (it % wandb_log_steps == 0):
            wandb.log(
                {
        ...
        if (
            checkpoint_steps
            and it > 0
            and (it % checkpoint_steps == 0)
            and args.output_dir
            and utils.is_main_process()
        ):
            output_dir = Path(args.output_dir)
            output_dir.mkdir(parents=True, exist_ok=True)
            checkpoint_path = output_dir / f"checkpoint_step_{it:07}.pth"
            ...
            if args.use_ema and ema_m is not None:
