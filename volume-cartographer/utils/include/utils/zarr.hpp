...
public:
    ...
    /// Check if an inner chunk has our explicit "verified-absent" sentinel
    /// (0xFF..FD, 0). Distinct from the zarr-spec empty sentinel — set only
    /// when we've performed a real, authenticated round-trip and the source
    /// confirmed the chunk does not exist (HTTP 404 or shard-index missing).
    [[nodiscard]] bool inner_chunk_is_verified_absent(std::span<const std::size_t> chunk_indices) const {
        if (!is_sharded()) return false;
        const auto ndim = meta_.ndim();
    ...
    /// Mark an inner chunk with our verified-absent sentinel (0xFF..FD, 0).
    /// Use this — not mark_inner_chunk_empty — for negative caching, so we
    /// can distinguish between zarr-spec empty markers (which a previous
    /// session might have written under bad auth) and chunks we positively
    /// verified absent on the source.
