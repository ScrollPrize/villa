...
- Folder: `volume-cartographer/scripts/tifxyz_label_transfer/`
- Language: Python 3.10+; platforms: Ubuntu/macOS, amd64/arm64
...
Repository agents should set `AGENTS_AGENT_MODE=1 AGENTS_ALLOW_INSTALL=1`
before running that install. No installation is needed if the packages are
already present.
...
AGENTS_AGENT_MODE=1 uv run \
  --with-requirements tifxyz_label_transfer/requirements.txt \
...
```bash
python3 volume-cartographer/scripts/tifxyz_label_transfer/transfer.py single \
...
```bash
python3 volume-cartographer/scripts/tifxyz_label_transfer/self_render_tifxyz.py \
  --case-dir /data/cases/pherc0139-w039 \
...
```bash
python3 volume-cartographer/scripts/tifxyz_label_transfer/estimate_canvas_offset.py \
  --source-tifxyz /data/old-2.4um.tifxyz \
  --target-tifxyz /data/updated-2.4um.tifxyz \
...
`ink-detection/koine_machines/preprocessing/create_label_zarrs.py`): the
...
dimension separators. Levels are derived in 2D and streamed block-wise, so
full-resolution canvases convert without materialising the 65-deep volume.

```bash
uv run --with numpy --with tifffile --with imagecodecs \
  --with "zarr<3" --with numcodecs \
  python tifxyz_label_transfer/make_label_zarrs.py \
...
uv run \
  --with-requirements \
  volume-cartographer/scripts/tifxyz_label_transfer/napari-requirements.txt \
  python \
  volume-cartographer/scripts/tifxyz_label_transfer/view_alignment_napari.py \
...
The native differential tests compile a fresh library in a temporary
directory and treat the NumPy rasterizer as the oracle: exact labels,
