...
def can_use_surface_index_backend(patches: Dict[str, Patch]) -> bool:
    try:
        import vc_surface_index  # noqa: F401
    except ImportError:
        return False
    return True
...
def _process_point_collections_with_surface_index(
    ...
    for patch_id, patch in patches.items():
        ...
        surface_defs.append(vc_surface_index.QuadSurface(patch_id, zyx, float(scale[0]), float(scale[1])))

    links: Dict[str, List[PointPatchLink]] = {}
    if not surface_defs or not point_collections:
        return links

    print(f'linking points to patches with vc_surface_index ({len(surface_defs)} patches)')
    index = vc_surface_index.SurfacePatchIndex()
