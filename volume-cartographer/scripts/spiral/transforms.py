...
class SpiralAndTransform(nn.Module):
    ...
    def get_slice_to_spiral_transform(self, truncate_at_step=None):
        ...
        diffeomorphisms = [
            IntegratedFlowDiffeomorphism(flow_field, self.flow_min_corner_zyx, self.flow_max_corner_zyx, num_steps=self.flow_integration_steps, solver=self.flow_integration_solver, truncate_at_step=truncate_at_step, rtol=self.cfg.get('flow_rtol', 1e-5), atol=self.cfg.get('flow_atol', 1e-6))
