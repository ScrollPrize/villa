...
def test_lasagna_store_predicates_reproduce_the_mode_contract():
    ...
    inference = {"dense_spacing_mode": "inference",
                 "loss_weight_dense_normals": 0.0}
    assert fit_input("winding_inference").required(inference) is True
    assert fit_input("winding_inference").enabled(inference) is True
    assert fit_input("normal_x").required(inference) is False
    assert fit_input("surf_sdt").required(inference) is False
