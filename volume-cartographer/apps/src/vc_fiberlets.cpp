...
void usage(const char* executable)
{
    std::cerr << "Usage:\n"
              ...
              << "  --verify-spatial-objective    verify centroid updates with legacy objective/backtracking\n"
...
CliOptions parseArgs(int argc, char** argv)
{
    ...
    for (int index = firstOption; index < argc; ++index) {
        ...
        } else if (argument == "--verify-spatial-objective" && options.command != Command::Paths) {
            options.anchors.verifySpatialObjective = true;
    ...
    const TubeExtractionResult& extraction)
{
    ...
    output << std::setprecision(17)
        << "fiberlet_extraction_profile version=29"
           ...
           << " anchor_fit_direct_centroid_acceptances="
           << fit.directCentroidAcceptances
