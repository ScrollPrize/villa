...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (dir_mode && is_zarr_root(in_path)) {
            ...
            std::unique_ptr<vc::zarr::Dataset> dsIn = [&]() -> std::unique_ptr<vc::zarr::Dataset> {
                std::string dimsep = ".";
                if (path_is_dataset) {
                    ...
                    return vc::zarr::Dataset::open(root, std::to_string(group_idx), dimsep);
                }
            }();
            ...
            if (!out_is_zarr) {
                ...
                for (long long zi = static_cast<long long>(z0); zi <= static_cast<long long>(z1); ++zi) {
                    ...
                    cv::Mat src;
                    // Read one Z slab
                    if (dsIn->getDtype() == vc::zarr::Datatype::uint8) {
                        xt::xarray<uint8_t> slab = xt::empty<uint8_t>({1ul, Y, X});
                        vc::zarr::ShapeType off = {z, 0ul, 0ul};
                        vc::zarr::readSubarray<uint8_t>(dsIn, slab, off.begin());
            ...
            auto dsOut0 = vc::zarr::Dataset::create(outFile, "0", "uint8", shape0, chunks0, "blosc", compOpts0);
            ...
            for (long long zi = static_cast<long long>(z0); zi <= static_cast<long long>(z1); ++zi) {
                ...
                vc::zarr::writeSubarray<uint8_t>(dsOut0, slabOut, offOut.begin());
            ...
            for (int targetLevel = 1; targetLevel <= 5; ++targetLevel) {
                auto src = vc::zarr::Dataset::open(outFile, std::to_string(targetLevel - 1));
                ...
                auto dst = vc::zarr::Dataset::create(outFile, std::to_string(targetLevel), "uint8", dShape, dChunks, "blosc", compOpts);
                ...
                for (long long ty = 0; ty < static_cast<long long>(tilesY); ++ty) {
                    for (long long tx = 0; tx < static_cast<long long>(tilesX); ++tx) {
                        ...
                        xt::xarray<uint8_t> srcChunk = xt::empty<uint8_t>({lz, ly*2ul, lx*2ul});
                        {
                            ...
                            vc::zarr::readSubarray<uint8_t>(src, srcChunk, off.begin());
                        }
                        ...
                        vc::zarr::writeSubarray<uint8_t>(dst, dstChunk, offD.begin());
                        size_t d = ++tilesDone;
                        #ifdef _OPENMP
                        if (omp_get_thread_num() == 0)
                        #endif
