...
Status: **protocol reference**. The compiled method descriptors are authoritative for
mechanical input schemas and MCP mappings; this document defines behavioral semantics.

The agent bridge is an in-process JSON-RPC 2.0 server inside VC3D, listening on a local
socket (`QLocalServer` / `QLocalSocket`). A separate MCP server process connects
...
Job ids are `"job-<n>"`, monotonically increasing across all sources. At most one
job per source may run at a time; independent sources may run concurrently. See §8.3
for the record shape, lifecycle authorities, retention, and progress delivery.
...
- **params:**
  ...
  `-32009 UNSUPPORTED` (see §8.1; use `segmentation.manual_add.*`). The C++ enum
...
One MCP server process exposes the tools below; each maps 1:1 onto an RPC
...
- New RPC families: `canvas.drag`, `segmentation.manual_add.*`,
  ...
  `tags.set`, `seeding.*`, `segmentation.push_pull.*`,
  `tracer.run_trace`).
...
### 8.1 `segmentation.grow` rejects `"manual_add"`
...
### 8.2 `catalog.open_sample` is always non-interactive

`MenuActionController::openOpenDataSample` shows a **blocking**
...
### 8.3 Source-tagged job model

The bridge allows **one active job per source**:
...
| `"catalog"` | open-data project load and prefill completion |
| `"flatten"` | `SegmentationCommandHandler::flattenJobStarted` / `flattenJobFinished` |
| `"seeding"` | `SeedingWidget::seedingBatchProgressChanged` / `seedingBatchFinished` |
| `"autosave"` | `SegmentationModule::autosaveCompleted` |
...
```json
{
  ...
  "source": "tool" | "growth" | "lasagna" | "atlas" | "catalog" | "flatten" | "seeding" | "autosave",
  ...
  "progressHistory": [obj],        // last <=64 sequenced job.progress payloads
...
- Retention: the last **8 completed** JobRecords **per source**.
...
- Every `job.progress` payload contains `jobId`, `source`, `kind`, `phase`, and a
  monotonically increasing per-job `seq`. The server retains the latest 64 payloads in
  `progressHistory`; clients may replay that tail and merge live notifications by `seq`.
  Delivery is ordered and best-effort for the lifetime of a wait call.
- `state.get` (§3.2): the `"job"` field keeps its v1 meaning (most recently started
  ...
  `"jobs": [JobRecord]` lists **all currently active** jobs.
...
- Responses to deferred methods may arrive **out of order** relative to later
  non-deferred requests on the same connection (an exception to the FIFO note in §1.2 for these
...
### 9.8 `state.get` editing fields

The `state.get` result includes four fields reporting manual-add and
correction-point-authoring state:
...
The documented precondition errors in §9.2 (`-32008`,
...
- **`MenuActionController` (§14.1).** The pre-existing single-purpose
...
- **result:** `{"requested": true}` — completion is observable via app status messages;
  this is deliberately not modeled as a job.
...
`LineAnnotationController` has three further headless
...
`activateSurfaceById` vs `selectSurfaceById`). Full list:
...
The signature includes a fourth out-param so
...
## 15. Tags, seeding, push/pull, and tracing
...
These RPCs use the dialog-free `SeedingWidget` entry points. Seed and path placement
reuses `canvas.click` and `canvas.drag`.
...
- **Batch execution.** Run and expand are non-blocking, keep their batch state in
  `SeedingWidget`, and drain through `QProcess::finished` callbacks.
  `runSegmentationHeadless` / `runExpandSeedsHeadless` share validation and launch
  behavior with the interactive slots without showing dialogs. Analyze-paths is
  synchronous; cancellation uses bounded terminate-then-kill teardown.
...
**Remote-aware volume argument.** `runSegmentationHeadless` and
`runExpandSeedsHeadless` resolve the volume through `commandPathForVolume`: a remote
volume contributes its
...
`startPushPull(int, std::optional<bool>)` and
...
### 18.4 Bridge: job source `"catalog"` and `catalog.open_sample`

Catalog loads use the `"catalog"` job source:
...
`catalog.open_sample` is asynchronous. Params,
...
reaches any of these dialogs. All three are **asynchronous** and share the job
...
### 20.1 Job source `"flatten"` and dialog suppression

The `"flatten"` source uses
two signals on `SegmentationCommandHandler`, emitted by all three job classes from
...
# Jobs, discovery, lifecycle, and viewer controls
...
## 22. Volume discovery (`volume.list`) and state
...
### 22.2 `state.get`: `volume.volumeIds`

The `volume` block includes a `"volumeIds": [str]` member (from
...
The family contains two synchronous dialog-free ops (`crop_bounds`, `recalc_area`), two
