    ...
    const std::shared_ptr<RemoteChunkFetchProgress>& progress)
{
    ...
    progress->plannedKeys.store(0, std::memory_order_relaxed);
    ...
    for (int row = 0; row < quadRows; ++row) {
        for (int col = 0; col < points->cols - 1; ++col) {
            ...
            for (const cv::Vec3f& point : {p01, p10, p11}) {
                lo[0] = std::min(lo[0], point[0]);
                lo[1] = std::min(lo[1], point[1]);
                lo[2] = std::min(lo[2], point[2]);
                hi[0] = std::max(hi[0], point[0]);
                hi[1] = std::max(hi[1], point[1]);
                hi[2] = std::max(hi[2], point[2]);
            }
    ...
    std::vector<vc::cache::ChunkKey> orderedKeys;
    ...
    progress->plannedKeys.store(orderedKeys.size(), std::memory_order_relaxed);
    ...
    const std::shared_ptr<RemoteChunkFetchProgress>& progress)
{
    ...
    if (!cache) {
        result.error = QObject::tr("Chunk cache is not available for the current volume.");
        return result;
    }
    ...
    progress->ioPending.store(cache->stats().ioPending, std::memory_order_relaxed);
    ...
    progress->availableKeys.store(keys.size(), std::memory_order_relaxed);
    progress->ioPending.store(0, std::memory_order_relaxed);
    return result;
}
...
void SegmentationCommandHandler::onFetchRemoteChunks(const std::string& segmentId)
{
    ...
    connect(&timer, &QTimer::timeout, &progress, [&progress, progressState, segmentName]() {
        ...
        if (totalKeys > 0) {
            progress.setRange(0, clampProgressCount(static_cast<qint64>(totalKeys)));
            progress.setValue(clampProgressCount(static_cast<qint64>(availableKeys)));
        } else {
            progress.setRange(0, 0);
        }
        progress.setLabelText(
    ...
    emit statusMessage(
        tr("Fetched %1 remote chunk(s) for '%2' across %3 scale level(s) (%4 already cached).")
            ...
            .arg(segmentName)
            .arg(result.levels)
            .arg(QString::number(static_cast<qulonglong>(result.initialAvailable))),
        7000);
}
