...
void CChunkedVolumeViewer::clearEditIntersectionItems()
{
    for (auto* item : _editIntersectionItems) {
        if (item && item->scene())
            _scene->removeItem(item);
        delete item;
    }
    _editIntersectionItems.clear();
}
...
void CChunkedVolumeViewer::renderEditedSurfaceIntersections(const cv::Rect& vertexRect)
{
    clearEditIntersectionItems();
    ...
    if (_volume && _chunkArray) {
        ...
        const int x0 = std::clamp(
            static_cast<int>(std::floor((vertexRect.x - padSurf - _camSurfX) * _camScale + fbW * 0.5f)),
            0, fbW);
        const int y0 = std::clamp(
            static_cast<int>(std::floor((vertexRect.y - padSurf - _camSurfY) * _camScale + fbH * 0.5f)),
            0, fbH);
        const int x1 = std::clamp(
            static_cast<int>(std::ceil((vertexRect.x + vertexRect.width + padSurf - _camSurfX) * _camScale + fbW * 0.5f)),
            0, fbW);
        const int y1 = std::clamp(
            static_cast<int>(std::ceil((vertexRect.y + vertexRect.height + padSurf - _camSurfY) * _camScale + fbH * 0.5f)),
            0, fbH);
        ...
        if (roiW > 0 && roiH > 0) {
            const int startLevel = renderStartLevel(true);
            const vc::render::ChunkedPlaneSampler::Options options(_samplingMethod, 32);
            const cv::Vec3f fullOffset(_surfacePtrX * _scale - float(fbW) * 0.5f,
                                       _surfacePtrY * _scale - float(fbH) * 0.5f,
                                       0.0f);
            const cv::Vec3f roiOffset = fullOffset + cv::Vec3f(float(x0), float(y0), 0.0f);

            cv::Mat_<cv::Vec3f> coords;
            cv::Mat_<cv::Vec3f> normals;
            activeSeg->gen(&coords, &normals, cv::Size(roiW, roiH), {0, 0, 0}, _scale, roiOffset);
            ...
            cv::Mat_<uint8_t> values(roiH, roiW, uint8_t(0));
            cv::Mat_<uint8_t> coverage(roiH, roiW, uint8_t(0));
            sampleCoordsIntoValues(coords, normals, startLevel, options, values, coverage);
            ...
            for (int y = 0; y < roiH; ++y) {
                auto* dst = fbBits + size_t(y0 + y) * size_t(fbStride) + x0;
                const auto* src = values.ptr<uint8_t>(y);
                const auto* cov = coverage.ptr<uint8_t>(y);
                for (int x = 0; x < roiW; ++x) {
                    dst[x] = cov[x] ? lut[src[x]] : 0xFF000000u;
                }
            }
        }
    }
    ...
    auto* points = activeSeg->rawPointsPtr();
    if (!points || points->empty()) {
        return;
    }

    cv::Rect bounds(0, 0, points->cols, points->rows);
    cv::Rect expanded(vertexRect.x - 2,
                      vertexRect.y - 2,
                      vertexRect.width + 4,
                      vertexRect.height + 4);
    cv::Rect clipped = expanded & bounds;
    if (clipped.empty()) {
        return;
    }
    ...
    cv::Vec3f low(std::numeric_limits<float>::max(),
                  ...
                  std::numeric_limits<float>::max());
    cv::Vec3f high(-std::numeric_limits<float>::max(),
                   -std::numeric_limits<float>::max(),
                   -std::numeric_limits<float>::max());
    bool any = false;
    for (int r = clipped.y; r < clipped.y + clipped.height; ++r) {
        const auto* row = points->ptr<cv::Vec3f>(r);
        for (int c = clipped.x; c < clipped.x + clipped.width; ++c) {
            ...
            for (int ax = 0; ax < 3; ++ax) {
                low[ax] = std::min(low[ax], p[ax]);
                high[ax] = std::max(high[ax], p[ax]);
            }
            any = true;
        }
    }
    ...
    const float scalePad = std::max(std::abs(activeSeg->scale()[0]), std::abs(activeSeg->scale()[1]));
    const float pad = std::max({scalePad * 4.0f, _intersectionThickness * 2.0f, 1.0f});
    for (int ax = 0; ax < 3; ++ax) {
        low[ax] -= pad;
        high[ax] += pad;
    }
    ...
    const float penWidth = std::max(_intersectionThickness, kActiveIntersectionMinWidthDelta);
    ...
    _editIntersectionItems.reserve(paths.size());
    for (size_t idx = 0; idx < paths.size(); ++idx) {
        ...
        item->setZValue(kActiveIntersectionZ + 1);
        item->setAcceptedMouseButtons(Qt::NoButton);
        _scene->addItem(item);
        _editIntersectionItems.push_back(item);
    }
    _view->viewport()->update();
}
