...
void CChunkedVolumeViewer::onVolumeClicked(QPointF scenePos, Qt::MouseButton button, Qt::KeyboardModifiers modifiers)
{
    if (_sameWrapAnnotation.enabled && button == Qt::LeftButton && modifiers.testFlag(Qt::ShiftModifier)) {
        ...
        if (generateSameWrapAnnotationPreview(scenePos, appendToPreview)) {
            return;
        }
    }
...
{
    _sameWrapAnnotation.enabled = enabled;
    if (!enabled) {
...
{
    _sameWrapAnnotation.spacingVx = std::max(1.0f, static_cast<float>(spacingVx));
}

void CChunkedVolumeViewer::clearSameWrapAnnotationPreview()
{
    _sameWrapAnnotation.componentScenePath.clear();
    _sameWrapAnnotation.componentVolumePath.clear();
    _sameWrapAnnotation.sampledVolumePoints.clear();
    _sameWrapAnnotation.hasPreview = false;
    _sameWrapAnnotation.shiftReleasedSincePreview = true;
    clearOverlayGroup(kSameWrapAnnotationOverlayKey);
}

bool CChunkedVolumeViewer::commitSameWrapAnnotationPreview()
{
    if (!_sameWrapAnnotation.enabled || !_sameWrapAnnotation.hasPreview ||
        _sameWrapAnnotation.sampledVolumePoints.empty() || !_pointCollection) {
        return false;
    }

    const std::string collectionName = _pointCollection->generateNewCollectionName("same_wrap");
    _pointCollection->addPoints(collectionName, _sameWrapAnnotation.sampledVolumePoints);
    clearSameWrapAnnotationPreview();
    return true;
}

bool CChunkedVolumeViewer::generateSameWrapAnnotationPreview(const QPointF& scenePos, bool appendToPreview)
{
    std::vector<QPointF> previousScenePath;
    std::vector<cv::Vec3f> previousVolumePath;
    std::vector<cv::Vec3f> previousSampledPoints;
    if (appendToPreview && _sameWrapAnnotation.hasPreview) {
        previousScenePath = _sameWrapAnnotation.componentScenePath;
        previousVolumePath = _sameWrapAnnotation.componentVolumePath;
        previousSampledPoints = _sameWrapAnnotation.sampledVolumePoints;
    } else {
        clearSameWrapAnnotationPreview();
    ...
    cv::Mat gray;
    if (!sampleSameWrapSourceImage(gray)) {
        if (appendToPreview) {
            _sameWrapAnnotation.componentScenePath = std::move(previousScenePath);
            _sameWrapAnnotation.componentVolumePath = std::move(previousVolumePath);
            _sameWrapAnnotation.sampledVolumePoints = std::move(previousSampledPoints);
            _sameWrapAnnotation.hasPreview = !_sameWrapAnnotation.sampledVolumePoints.empty();
        }
        return false;
    }
    ...
    const float spacingPx = std::max(1.0f, _sameWrapAnnotation.spacingVx * _scale);
    ...
    if (appendToPreview && !previousSampledPoints.empty() && !sampled.empty()) {
        if (cv::norm(sampled.front() - previousSampledPoints.back()) <
            std::max(0.5f, _sameWrapAnnotation.spacingVx * 0.5f)) {
        ...
        _sameWrapAnnotation.componentScenePath = std::move(scenePath);
        _sameWrapAnnotation.componentVolumePath = std::move(volumePath);
        _sameWrapAnnotation.sampledVolumePoints = std::move(sampled);
    }
    _sameWrapAnnotation.clickScenePos = QPointF(clickX, clickY);
    _sameWrapAnnotation.hasPreview = true;
    _sameWrapAnnotation.shiftReleasedSincePreview = false;
    updateSameWrapAnnotationOverlay();
    return true;
}

bool CChunkedVolumeViewer::sampleSameWrapSourceImage(cv::Mat& gray) const
{
    if (_framebuffer.isNull() || _framebuffer.width() <= 0 || _framebuffer.height() <= 0) {
        return false;
    }

    const QImage image = _framebuffer.convertToFormat(QImage::Format_RGB32);
...
void CChunkedVolumeViewer::updateSameWrapAnnotationOverlay()
{
    ...
    if (_sameWrapAnnotation.componentVolumePath.size() >= 2) {
        QPainterPath path(volumeToScene(_sameWrapAnnotation.componentVolumePath.front()));
        for (size_t i = 1; i < _sameWrapAnnotation.componentVolumePath.size(); ++i) {
            path.lineTo(volumeToScene(_sameWrapAnnotation.componentVolumePath[i]));
        }
    ...
    for (const auto& point : _sameWrapAnnotation.sampledVolumePoints) {
    ...
    auto* clickMarker = new QGraphicsEllipseItem(_sameWrapAnnotation.clickScenePos.x() - 6.0,
                                                 _sameWrapAnnotation.clickScenePos.y() - 6.0,
                                                 12.0,
                                                 12.0);
