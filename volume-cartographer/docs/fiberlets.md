...
window. The default fast path accepts this bounded centroid directly. Pass
`--verify-spatial-objective` to evaluate the previous and proposed positions
with the retained spatial objective and use deterministic position-only
backtracking at fractions `1, 1/2, ...` through the first displacement at or
below the configured peak-grid step. Verification accepts the first strict
improvement and otherwise retains the projected baseline. This quality knob is
substantially slower because every candidate scans the complete support.
...
The following direction-conditioned peak response uses a 2049-entry nearest
lookup for its transverse and axial `exp(-x)` values over normalized exponent
`[0,8]`, after their exact support cutoffs. Values outside the table domain
retain `exp`; the default three-sigma support ends at exponent `4.5`. Direct
tests bound relative lookup error below `0.21%`. This approximation is limited
to peak search; broad direction fitting and final support retain their existing
exponential paths.
...
Public scoring plus source and sink transitions use squared angular distance.
For a valid normal, reused interior DP transitions instead split each unit
direction into scalar normal projection and unnormalized tangent-plane vector,
then weight their squared component differences. Equal weights reconstruct the
full squared direction chord. Invalid normals use isotropic squared chordal
distance. Nonzero free angles are converted to chord length before component
subtraction. This intentionally trades small interior cost differences for
avoiding inverse trigonometry and, at the zero-free-angle default, all per-turn
square roots and divisions. Cumulative history smoothness remains excluded
from the DP state.
...
Profile version 26 reports `anchor_fit_direct_centroid_acceptances`. In the
default fast mode this counts bounded retained-evidence centroids installed
without spatial-objective verification; refined-state evaluation and
backtracking counters remain zero. With `--verify-spatial-objective`, direct
acceptances remain zero and the existing candidate, depth, evaluation, and
backtracking counters describe the verified path.
Prepared proposal records store position relative to the fixed cell pivot. The
compact proposal kernel loads that offset once per pass and reuses it with
precomputed component-to-pivot offsets across component support and Gaussian
calculations. Expanded/public anchor fitting retains its generic geometry path.
Only finite generated positions enter this private prepared stream, so compact
proposal passes do not repeat position validation. Invalid arbitrary input is
still handled by the checked expanded/detail fitting paths.
