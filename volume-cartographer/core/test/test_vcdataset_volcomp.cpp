...
// End-to-end through the recompress tool: synthesize a zarr v2 raw volume,
// run `vc_zarr_recompress --codec volcomp`, open the v3 result with the
// pyramid opener and VcDataset, and check the read-back quality. Opt in with
// VC_RECOMPRESS_BIN=<path to vc_zarr_recompress>.
TEST_CASE("volcomp: vc_zarr_recompress --codec volcomp end to end (opt-in)")
{
    const char* bin = std::getenv("VC_RECOMPRESS_BIN");
    if (!bin || !utils::volcomp_available()) {
        MESSAGE("Skipping: set VC_RECOMPRESS_BIN to run");
        return;
    }
    auto d = tmpDir("recompress");
    // zarr v2 level 0: 256^3, 128^3 raw chunks, "/" separator; 7 of 8 present
    auto in = d / "in.zarr";
    fs::create_directories(in / "0");
    {
        std::ofstream z(in / "0" / ".zarray");
        z << R"({"zarr_format":2,"shape":[256,256,256],"chunks":[128,128,128],"dtype":"|u1",)"
             R"("compressor":null,"fill_value":0,"order":"C","filters":null,"dimension_separator":"/"})";
        std::ofstream g(in / ".zgroup");
        g << R"({"zarr_format":2})";
        std::ofstream a(in / ".zattrs");
        a << R"({"multiscales":[{"version":"0.4","axes":[{"name":"z","type":"space"},{"name":"y","type":"space"},{"name":"x","type":"space"}],)"
             R"("datasets":[{"path":"0","coordinateTransformations":[{"type":"scale","scale":[1,1,1]}]}]}]})";
    }
    std::vector<std::vector<uint8_t>> srcs;
    for (int z = 0; z < 2; ++z)
        for (int y = 0; y < 2; ++y)
            for (int x = 0; x < 2; ++x) {
                if (z == 1 && y == 1 && x == 1) continue;  // absent chunk
                auto v = synth(uint32_t(z * 4 + y * 2 + x + 1));
                srcs.push_back(v);
                fs::create_directories(in / "0" / std::to_string(z) / std::to_string(y));
                std::ofstream f(in / "0" / std::to_string(z) / std::to_string(y) / std::to_string(x),
                                std::ios::binary);
                f.write(reinterpret_cast<const char*>(v.data()), std::streamsize(v.size()));
            }
    auto out = d / "out.zarr";
    std::string cmd = std::string(bin) + " " + in.string() + " " + out.string() +
                      " --codec volcomp --q 8 --jobs 1 --levels 0 --stats-pct 100 > " +
                      (d / "recompress.log").string() + " 2>&1";
    const int rc = std::system(cmd.c_str());
    {
        std::ifstream l(d / "recompress.log");
        std::string log((std::istreambuf_iterator<char>(l)), {});
        MESSAGE(log);
    }
    REQUIRE(rc == 0);
    REQUIRE(fs::exists(out / "0" / "zarr.json"));

    vc::VcDataset ds(out / "0");
    REQUIRE(ds.defaultChunkShape()[0] == 128);
    ...
    for (int z = 0; z < 2; ++z)
        for (int y = 0; y < 2; ++y)
            for (int x = 0; x < 2; ++x) {
                if (z == 1 && y == 1 && x == 1) {
                    std::fill(buf.begin(), buf.end(), 9);
                    CHECK_FALSE(ds.readChunkOrFill(1, 1, 1, buf.data()));
                    CHECK(std::all_of(buf.begin(), buf.end(), [](uint8_t v) { return v == 0; }));
                    continue;
                }
                REQUIRE(ds.readChunk(std::size_t(z), std::size_t(y), std::size_t(x), buf.data()));
                const double p = psnr(srcs[k++], buf);
                CHECK(p > 38.0);
            }
    auto opened = vc::render::openLocalZarrPyramid(out);
    REQUIRE(opened.fetchers.size() == 1);
    CHECK(opened.chunkShapes[0] == std::array<int, 3>{128, 128, 128});
    CHECK(opened.storageChunkShapes[0] == std::array<int, 3>{1024, 1024, 1024});
    vc::render::ChunkKey key{};
    key.iz = 1; key.iy = 0; key.ix = 1;
    auto res = opened.fetchers[0]->fetch(key);
    REQUIRE(res.status == vc::render::ChunkFetchStatus::Found);
    CHECK(res.hasPersistentBytes);
    CHECK(utils::is_volcomp_compressed(res.persistentBytes));
    fs::remove_all(d);
