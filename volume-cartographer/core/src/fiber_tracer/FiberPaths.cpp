...
struct DpEdge {
    ...
    detail::FiberLocalPreparedProjectedChordalCandidate candidate;
};
    ...
    SolveProfile& profile)
{
    ...
    const float smoothnessFreeChord = 2.0F * std::sin(
        metricConfig.smoothness.freeAngleRadians * 0.5F);
        ...
        for (size_t node = currentRange.begin; node < currentRange.end; ++node) {
            ...
            for (size_t previousState = 0; previousState < stateCount; ++previousState) {
                ...
                for (size_t lane = 0; lane < outgoingAlignment.count; ++lane) {
                    ...
                    nextCost +=
                        detail::fiberLocalMetricCostFromPreparedAlignmentProjectedChordalInline(
