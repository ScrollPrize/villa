...
std::vector<cv::Vec3f> reconstructedStoredRoutePoints(
    const FiberletStoredPrefix& prefix,
    const FiberletStoredRoute& route,
    const FiberletStoredAnchor& firstAnchor,
    const FiberletStoredAnchor& secondAnchor,
    const FiberletPathConfig& pathConfig)
{
    std::vector<std::array<std::int16_t, 2>> lattice;
    if (prefix.interiorPointCount == 1) {
        lattice.push_back(prefix.entryUV);
    } else if (prefix.interiorPointCount >= 2) {
        if (route.middleUV.size() + 2 != prefix.interiorPointCount) {
            throw std::runtime_error(
                "fiberlet route interior count differs from its prefix");
        }
        lattice.push_back(prefix.entryUV);
        lattice.insert(
            lattice.end(), route.middleUV.begin(), route.middleUV.end());
        lattice.push_back(prefix.exitUV);
    } else if (!route.middleUV.empty()) {
        throw std::runtime_error(
            "direct fiberlet route unexpectedly has interior geometry");
    }
    const auto expectedCosts =
        static_cast<std::size_t>(prefix.interiorPointCount) + 1;
    if (route.segmentCostDensities.size() != expectedCosts) {
        throw std::runtime_error(
            "fiberlet route cost-density count differs from its geometry");
    }
    return reconstructFiberletRoutePoints(
        firstAnchor.positionPredictionXYZ,
        firstAnchor.fittedAxisXYZ,
        secondAnchor.positionPredictionXYZ,
        secondAnchor.fittedAxisXYZ,
        lattice,
        pathConfig);
}
...
static FiberletFilterPlan planFiberletFilterStagesImpl(
    std::span<const FiberletFilterStageSpec> stages,
    std::span<const cv::Vec3d> referenceBaseXYZ,
    double corridorRadiusBaseVoxels,
    const std::optional<cv::Vec3d>& targetMinimumBaseXYZ,
    const std::optional<cv::Vec3d>& targetMaximumBaseXYZ,
    ...
    const FiberletChunkRouteAnalysisConfig& analysisTemplate)
{
    if (stages.empty())
        return {};
    const bool boxSelection = targetMinimumBaseXYZ.has_value() ||
        targetMaximumBaseXYZ.has_value();
    if (targetMinimumBaseXYZ.has_value() !=
        targetMaximumBaseXYZ.has_value()) {
        throw std::invalid_argument("fiberlet filter target box is incomplete");
    }
    if (!boxSelection &&
        (referenceBaseXYZ.empty() || !(corridorRadiusBaseVoxels > 0.0) ||
    ...
    if (boxSelection) {
        for (int axis = 0; axis < 3; ++axis) {
            if (!std::isfinite((*targetMinimumBaseXYZ)[axis]) ||
                !std::isfinite((*targetMaximumBaseXYZ)[axis]) ||
                !((*targetMaximumBaseXYZ)[axis] >
                  (*targetMinimumBaseXYZ)[axis])) {
                throw std::invalid_argument(
                    "fiberlet filter target box is invalid");
            }
        }
    }
    ...
    if (boxSelection) {
        insertIntersecting(
            finalGrid, *targetMinimumBaseXYZ, *targetMaximumBaseXYZ,
            selected.back(), [&](const auto& box) {
                for (int axis = 0; axis < 3; ++axis) {
                    if (box.maximumBaseXYZ[axis] <=
                            (*targetMinimumBaseXYZ)[axis] ||
                        box.minimumBaseXYZ[axis] >=
                            (*targetMaximumBaseXYZ)[axis]) {
                        return false;
                    }
                }
    ...
    } else if (referenceBaseXYZ.size() == 1) {
...
FiberletFilterPlan planFiberletFilterStages(
    ...
    const FiberletChunkRouteAnalysisConfig& analysisTemplate)
{
    return planFiberletFilterStagesImpl(
        stages, referenceBaseXYZ, corridorRadiusBaseVoxels, std::nullopt,
        std::nullopt, volumeMaximumBaseXYZ,
        maximumEndpointReachBaseXYZ, analysisTemplate);
}

FiberletFilterPlan planFiberletFilterStagesForBox(
    std::span<const FiberletFilterStageSpec> stages,
...
{
    return planFiberletFilterStagesImpl(
        stages, {}, 0.0, minimumBaseXYZ, maximumBaseXYZ,
        volumeMaximumBaseXYZ, maximumEndpointReachBaseXYZ,
        analysisTemplate);
}
...
void FiberletChunkGraphSource::prefetchAnchorChunks(
    std::span<const vc::render::ChunkKey> chunks,
    bool blocking) const
{
    anchorCache_->prefetchChunks(
        std::vector<vc::render::ChunkKey>(chunks.begin(), chunks.end()),
        blocking);
}

void FiberletChunkGraphSource::prefetchFiberletChunks(
    std::span<const vc::render::ChunkKey> chunks,
    bool blocking) const
{
    fiberletCache_->prefetchChunks(
        std::vector<vc::render::ChunkKey>(chunks.begin(), chunks.end()),
        blocking);
}
    ...
    const FiberletStorageId& fiberlet, bool blocking) const
{
    ...
    result.value.anchorPayloadLeases = {firstAnchor.value.payloadLease, secondAnchor.value.payloadLease};
    try {
        result.value.pointsPredictionXYZ = reconstructedStoredRoutePoints(
            result.value.prefix, result.value.route,
            firstAnchor.value.anchor, secondAnchor.value.anchor,
            pathConfig_);
    } catch (const std::exception& error) {
        result.status = FiberletGraphQueryStatus::Error;
        const auto& first = fiberlet.first;
        const auto& second = fiberlet.second;
        result.error = std::string(error.what()) +
            " fiberlet=" +
            std::to_string(first.coordinateZYX[0]) + ',' +
            std::to_string(first.coordinateZYX[1]) + ',' +
            std::to_string(first.coordinateZYX[2]) + ',' +
            std::to_string(first.variant) + "->" +
            std::to_string(second.coordinateZYX[0]) + ',' +
            std::to_string(second.coordinateZYX[1]) + ',' +
            std::to_string(second.coordinateZYX[2]) + ',' +
            std::to_string(second.variant);
    }
    return result;
}
...
const FiberletPathConfig& FiberletChunkGraphSource::pathConfig() const noexcept
{
    return pathConfig_;
}
...
FiberletStoredReplayGraphSource::FiberletStoredReplayGraphSource(
    ...
    FiberletPathConfig pathConfig,
    float maximumJoinAngleDegrees)
    : dataset_(std::move(fiberletDataset))
    , anchorCache_(std::move(anchorCache))
    , pathCache_(std::move(fiberletCache))
    , pathConfig_(std::move(pathConfig))
    , chunks_(
          std::move(anchorDataset), anchorCache_, dataset_, pathCache_,
          pathConfig_)
    , maximumJoinAngleDegrees_(maximumJoinAngleDegrees)
{
    if (!dataset_ ||
        dataset_->metadata().kind != FiberletDatasetKind::Fiberlets) {
        throw std::invalid_argument(
            "layered stored replay graph requires a Fiberlet path dataset");
    }
    validateFiberletPathConfig(pathConfig_);
    if (!(maximumJoinAngleDegrees_ >= 0.0F) ||
        !(maximumJoinAngleDegrees_ <= 180.0F) ||
...
struct ChunkRouteLocalGraph {
    ...
    std::vector<FiberletStoredAnchor> seedAnchors;
    ...
    FiberletGraphMaterializationDiagnostics* diagnostics = nullptr)
{
    ...
    for (const auto& [key, anchor] : insideAnchors) {
        result.seedAnchors.push_back(anchor);
        insideKeys.push_back(key);
    }

    std::map<FiberletStorageId, FiberletStoredPrefix> physicalPrefixes;
    using StoredRouteReference = std::pair<
        std::shared_ptr<const FiberletRouteChunkPayload>, std::size_t>;
    std::map<FiberletStorageId, StoredRouteReference> physicalRoutes;
    ...
    if (buildReplayGeometry) {
        ...
        for (auto owner : prefixOwners) {
            owner.level = 1;
            pathChunks.push_back(owner);
        }
    }
    graph.prefetchFiberletChunks(pathChunks, true);
    for (const auto& chunk : prefixOwners) {
        ...
        if (buildReplayGeometry) {
            const auto loadedRoutes = graph.routesInChunk(chunk, true);
            if (loadedRoutes.status != FiberletGraphQueryStatus::Ready) {
                throw std::runtime_error(
                    "fiberlet crop route chunk failed: " +
                    loadedRoutes.error);
            }
            routes = loadedRoutes.value.payloadLease;
            if (routes->routes.size() !=
                loaded.value.payloadLease->prefixes.size()) {
                throw std::runtime_error(
    ...
    std::vector<vc::render::ChunkKey> endpointOwnerChunks;
    ...
    if (buildReplayGeometry) {
        for (auto iterator = physicalPrefixes.begin();
             iterator != physicalPrefixes.end();) {
            const auto first = endpointAnchors.find(iterator->first.first);
            const auto second = endpointAnchors.find(iterator->first.second);
            const auto endpointInside = [&](const auto& found) {
                return found != endpointAnchors.end() && pointInsideChunk(
                    cv::Vec3d(found->second.positionPredictionXYZ) *
                        predictionToBaseScale,
                    config);
            };
            const bool incident = endpointInside(first) || endpointInside(second);
            if (incident &&
                (first == endpointAnchors.end() ||
                 second == endpointAnchors.end())) {
                throw std::runtime_error(
            ...
            if (incident) {
                ...
                physicalRoutes.erase(iterator->first);
                iterator = physicalPrefixes.erase(iterator);
            }
        }
    } else if (endpointAnchors.size() != requiredAnchorKeys.size()) {
        throw std::runtime_error(
            "fiberlet graph endpoint anchor is absent from its owner chunk");
    }
    ...
    // Crop tracing receives every in-bounds anchor as a seed. Preserve
    // degree-zero seeds in the immutable graph so querying their adjacency
    // yields an empty range instead of treating the anchor as absent.
    if (buildReplayGeometry) {
        for (const auto& [key, anchor] : insideAnchors) {
            (void)key;
            ensureAnchor(anchor);
    ...
    for (const auto& id : result.physicalFiberlets) {
        ...
        if (buildReplayGeometry) {
            const auto route = physicalRoutes.find(id);
            if (route == physicalRoutes.end()) {
                throw std::logic_error(
                    "fiberlet crop bulk materialization lost route geometry");
            }
            const auto& storedRoute =
                route->second.first->routes.at(route->second.second);
            std::vector<cv::Vec3f> pointsPrediction;
            try {
                pointsPrediction = reconstructedStoredRoutePoints(
                    prefix->second, storedRoute, firstAnchor->second,
                    secondAnchor->second, graph.pathConfig());
            } catch (const std::exception& error) {
                throw std::runtime_error(
                    std::string(error.what()) + " fiberlet=" +
                    std::to_string(id.first.coordinateZYX[0]) + ',' +
                    std::to_string(id.first.coordinateZYX[1]) + ',' +
                    std::to_string(id.first.coordinateZYX[2]) + ',' +
                    std::to_string(id.first.variant) + "->" +
                    std::to_string(id.second.coordinateZYX[0]) + ',' +
                    std::to_string(id.second.coordinateZYX[1]) + ',' +
                    std::to_string(id.second.coordinateZYX[2]) + ',' +
                    std::to_string(id.second.variant));
            }
            FiberletReplaySourceCostProfile profile;
            profile.segmentCostDensities = storedRoute.segmentCostDensities;
            profile.segmentLengthsPredictionVoxels.reserve(
                pointsPrediction.size() - 1);
            std::vector<cv::Vec3d> pointsBase;
            pointsBase.reserve(pointsPrediction.size());
            for (const auto& point : pointsPrediction)
                pointsBase.emplace_back(point * predictionToBaseScale);
            for (std::size_t index = 1; index < pointsPrediction.size();
                 ++index) {
                const float segmentLength = vectorLength(
                    pointsPrediction[index] - pointsPrediction[index - 1]);
                if (!(segmentLength > 0.0F) ||
                    !std::isfinite(segmentLength)) {
                    throw std::runtime_error(
                        "stored Fiberlet route segment length is invalid");
                }
                profile.segmentLengthsPredictionVoxels.push_back(
                    segmentLength);
            }
    ...
    const FiberletChunkRouteAnalysisConfig& config)
{
    ...
    const auto local = materializeChunkRouteGraph(
        graph, config, seeds, false, false);
    ...
    std::span<const FiberletStorageId> retainedPhysicalFiberlets)
{
    ...
    std::vector<vc::render::ChunkKey> seedChunks;
    const auto local = materializeChunkRouteGraph(
        graph, config, seedChunks, true, false);
...
FiberletTransientLayer createStoredCombinedFiberletLayerView(
    const std::filesystem::path&,
    const std::shared_ptr<FiberletChunkDataset>& combined,
    const FiberletChunkCacheOptions& cacheOptions)
{
    if (!combined ||
        combined->metadata().kind != FiberletDatasetKind::Combined) {
        throw std::invalid_argument(
            "stored transient Fiberlet view requires a combined dataset");
    }
    return {
        combined, createStoredFiberletAnchorChunkCache(combined, cacheOptions),
        combined, createStoredFiberletPathChunkCache(combined, cacheOptions)};
}

FiberletTransientLayer applyTransientFiberletReductionStage(
...
{
    if (!previous.anchors || !previous.anchorCache || !previous.fiberlets ||
        !previous.fiberletCache || boxes.empty()) {
        throw std::invalid_argument(
            "transient Fiberlet reduction stage is invalid");
    }
    ...
    // Reduction repeatedly rewrites subsets. Promote rewritten chunks to the
    // lossless cache profile so compact directions and costs are never
    // requantized as they pass through successive transient layers.
    anchorMetadata.profile = FiberletStorageProfile::Float32Cache;
    anchorMetadata.costBits = 32;
    anchorMetadata.positionQuantumBaseVoxels = 0;
    fiberletMetadata.profile = FiberletStorageProfile::Float32Cache;
    fiberletMetadata.costBits = 32;
    fiberletMetadata.positionQuantumBaseVoxels = 0;
    anchorMetadata.processing["reduction"] = reduction;
    anchorMetadata.processing["reduction"]["source_dataset_fingerprint"] =
        previous.anchors->metadata().datasetFingerprint;
    fiberletMetadata.processing["reduction"] = std::move(reduction);
    fiberletMetadata.processing["reduction"]["source_dataset_fingerprint"] =
        previous.fiberlets->metadata().datasetFingerprint;
    ...
    for (std::size_t boxIndex = 0; boxIndex < boxes.size(); ++boxIndex) {
        ...
        const std::clock_t writeCpuStarted = std::clock();
        ...
        const double writeCpuSeconds = static_cast<double>(
            std::clock() - writeCpuStarted) / CLOCKS_PER_SEC;
        if (boxCompleted) {
            boxCompleted(
                boxIndex, reduced, written, writeWallSeconds,
                writeCpuSeconds);
        }
