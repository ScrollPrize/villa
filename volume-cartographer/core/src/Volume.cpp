...
void Volume::populateAutoGeneratedMetadata()
{
    const auto baseName = path_.filename().string();
    metadata_ = utils::Json::object();
...
void Volume::loadMetadata()
{
    ...
    if (std::filesystem::exists(metaPath)) {
        ...
        if (!full.contains("scan")) {
            // metadata.json exists but isn't a VC scan-metadata file
            // (e.g. surface-inference / workflow manifests that share the
            // name). Treat as if no metadata file were present.
            populateAutoGeneratedMetadata();
            return;
        }
        metadata_ = full;
