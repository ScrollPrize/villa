...
void Volume::zarrOpen()
{
    ...
    // Parse .zattrs once to get per-directory scale factors.
    // Reused for auto-gen dimensions, shape validation, and runtime scale lookup.
    {
        std::unordered_map<std::string, double> dirSf;
        auto attrs = vc::readZarrAttributes(path_);
        if (attrs.contains("multiscales") && attrs["multiscales"].is_array()) {
            for (auto& ms : attrs["multiscales"]) {
                if (!ms.contains("datasets") || !ms["datasets"].is_array()) continue;
                for (auto& ds_entry : ms["datasets"]) {
                    if (!ds_entry.contains("path")) continue;
                    std::string p = ds_entry["path"].get_string();
                    if (ds_entry.contains("coordinateTransformations")) {
                        for (auto& ct : ds_entry["coordinateTransformations"]) {
                            if (ct.contains("type") && ct["type"].get_string() == "scale"
                                && ct.contains("scale") && ct["scale"].is_array()
                                && ct["scale"].size() >= 1) {
                                dirSf[p] = ct["scale"].at(0).get_float();
                            }
                        }
                    }
                }
        ...
        // Helper: resolve scale factor for a directory name
        auto sfFor = [&](const std::string& dirName) -> double {
            auto it = dirSf.find(dirName);
            if (it != dirSf.end()) return it->second;
            try { return static_cast<double>(size_t{1} << std::stoi(dirName)); } catch (...) {}
            return 1.0;
        };

        // Build zarrScaleFactors_ (used by Slicing.cpp at runtime)
        zarrScaleFactors_.resize(zarrDs_.size());
        for (size_t i = 0; i < zarrDs_.size(); ++i) {
            zarrScaleFactors_[i] = static_cast<float>(
                sfFor(zarrDs_[i]->path().filename().string()));
        }

        // Auto-generate volume dimensions from zarr shapes + scale factors
        if (metadataAutoGenerated_) {
            bool hasRef = false;
            for (size_t i = 0; i < zarrDs_.size(); ++i) {
                if (!zarrDs_[i]) continue;
                const auto& shape = zarrDs_[i]->shape();
                double sf = zarrScaleFactors_[i];
                if (!hasRef) {
                    _slices = static_cast<int>(std::round(shape[0] * sf));
                    _height = static_cast<int>(std::round(shape[1] * sf));
                    _width  = static_cast<int>(std::round(shape[2] * sf));
                    hasRef = true;
                }
            }
            if (!hasRef)
            ...
            std::fprintf(stderr, "[Volume] %s: auto-generated dims [z,y,x]=(%d, %d, %d) from %zu scale(s)\n",
                         path_.filename().string().c_str(), _slices, _height, _width, zarrDs_.size());
        }
        ...
        if (!skipShapeCheck) {
            ...
            for (size_t i = 0; i < zarrDs_.size(); ++i) {
                ...
                double sf = zarrScaleFactors_[i];
                const auto& shape = zarrDs_[i]->shape();
                int eSlices = static_cast<int>(std::ceil(_slices / sf));
                int eHeight = static_cast<int>(std::ceil(_height / sf));
                int eWidth  = static_cast<int>(std::ceil(_width / sf));
                auto dirName = zarrDs_[i]->path().filename().string();
                if (int(shape[0]) != eSlices || int(shape[1]) != eHeight || int(shape[2]) != eWidth) {
                    throw std::runtime_error(
                        "zarr level '" + dirName + "' (scale=" + std::to_string(sf) + ") shape [z,y,x]=("
                        + std::to_string(shape[0]) + ", " + std::to_string(shape[1]) + ", " + std::to_string(shape[2])
                        + ") does not match expected (slices=" + std::to_string(eSlices)
                        + ", height=" + std::to_string(eHeight) + ", width=" + std::to_string(eWidth)
                        + ") in " + path_.string());
                }
            }
            if (!hasAny)
...
float Volume::levelScaleFactor(int vectorIndex) const noexcept {
    if (vectorIndex < 0 || static_cast<size_t>(vectorIndex) >= zarrScaleFactors_.size())
        return static_cast<float>(size_t{1} << vectorIndex);  // fallback
    return zarrScaleFactors_[static_cast<size_t>(vectorIndex)];
}
...
std::unique_ptr<vc::cache::BlockPipeline> Volume::createTieredCache() const
{
    ...
    auto pipeline = std::make_unique<vc::cache::BlockPipeline>(
        ...
        std::move(decompress),
        std::move(diskLevels));
    if (!zarrScaleFactors_.empty())
        pipeline->setLevelScaleFactors(zarrScaleFactors_);
    return pipeline;
}
