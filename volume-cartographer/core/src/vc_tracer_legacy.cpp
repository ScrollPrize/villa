...
static nlohmann::json legacy_json_from_utils(const utils::Json& json)
{
    return nlohmann::json::parse(json.dump());
}

static utils::Json legacy_json_to_utils(const nlohmann::json& json)
{
...
public:
    ...
    explicit SurfaceMeta(QuadSurface* surf)
        : SurfaceMeta()
    {
        setBorrowedSurface(surf);
    }
    ...
    explicit SurfaceMeta(const std::filesystem::path& path_)
        : SurfaceMeta()
    {
        path = path_;
        _meta_storage = nlohmann::json::parse(utils::Json::parse_file(path / "meta.json").dump());
    ...
    {
        if (!_surf) {
            _owned_surf = load_quad_from_tifxyz(path.string());
            _surf = _owned_surf.get();
            syncFromSurface();
        }
        return _surf;
    }
    ...
    {
        _owned_surf.reset(surf);
        _surf = surf;
        syncFromSurface();
    }

    void setBorrowedSurface(QuadSurface* surf)
    {
        _owned_surf.reset();
        _surf = surf;
        syncFromSurface();
...
private:
    void syncFromSurface()
    {
        if (!_surf)
            return;
        path = _surf->path;
        _meta_storage = legacy_json_from_utils(_surf->meta);
        overlapping_str = _surf->overlappingIds();
        meta = &_meta_storage;
    }

    nlohmann::json _meta_storage = nlohmann::json::object();
    std::unique_ptr<QuadSurface> _owned_surf;
};
...
static QuadSurface *grow_surf_from_surfs_legacy_impl(SurfaceMeta *seed, const std::vector<SurfaceMeta*> &surfs_v, const nlohmann::json &params, float voxelsize)
{
...
#pragma omp critical
        ...
        if (generation % 50 == 0 || update_mapping /*|| generation < 10*/) {
            {
                ...
                dbg_surf->meta["vc_grow_seg_from_segments_params"] = legacy_json_to_utils(params);
                ...
                dbg_surf->meta["used_approved_segments"] = legacy_json_to_utils(nlohmann::json(std::vector<std::string>(used_approved_names.begin(), used_approved_names.end())));
    ...
    surf->meta["used_approved_segments"] = legacy_json_to_utils(nlohmann::json(std::vector<std::string>(used_approved_names.begin(), used_approved_names.end())));

    return surf;
}

QuadSurface *grow_surf_from_surfs_legacy(QuadSurface *seed, const std::vector<QuadSurface*> &surfs_v, const utils::Json &params, float voxelsize)
{
    if (!seed)
        return nullptr;

    std::vector<std::unique_ptr<SurfaceMeta>> owned;
    std::vector<SurfaceMeta*> legacy_surfs;
    std::unordered_map<QuadSurface*, SurfaceMeta*> by_surface;

    auto add_surface = [&](QuadSurface* surf) -> SurfaceMeta* {
        if (!surf)
            return nullptr;
        auto it = by_surface.find(surf);
        if (it != by_surface.end())
            return it->second;
        owned.push_back(std::make_unique<SurfaceMeta>(surf));
        SurfaceMeta* meta = owned.back().get();
        by_surface.emplace(surf, meta);
        legacy_surfs.push_back(meta);
        return meta;
    };

    SurfaceMeta* seed_meta = nullptr;
    for (QuadSurface* surf : surfs_v) {
        SurfaceMeta* meta = add_surface(surf);
        if (surf == seed)
            seed_meta = meta;
    }

    if (!seed_meta)
        seed_meta = add_surface(seed);

    nlohmann::json legacy_params = legacy_json_from_utils(params);
    return grow_surf_from_surfs_legacy_impl(seed_meta, legacy_surfs, legacy_params, voxelsize);
}
