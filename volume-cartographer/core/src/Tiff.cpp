...
xt::xarray<float> read3DTiff(const std::filesystem::path& path)
{
    ...
    // Count directories (pages/slices)
    int depth = 0;
    do {
        ++depth;
    ...
    // Get dimensions from first directory
    ...
    if (spp != 1) {
        TIFFClose(tiff);
        throw std::runtime_error("Expected single-channel TIFF, got " +
                                 std::to_string(spp) + " channels: " + path.string());
    }

    if (bps != 32 || sampleFormat != SAMPLEFORMAT_IEEEFP) {
        TIFFClose(tiff);
        throw std::runtime_error("Expected 32-bit float TIFF, got " +
                                 std::to_string(bps) + " bits, sampleFormat=" +
                                 std::to_string(sampleFormat) + ": " + path.string());
    }

    // Allocate output array (depth, height, width) = ZYX order
    xt::xarray<float> result = xt::empty<float>({
        static_cast<size_t>(depth),
        static_cast<size_t>(height),
        static_cast<size_t>(width)
    });

    // Read each directory/page as a 2D slice
    for (int z = 0; z < depth; ++z) {
        TIFFSetDirectory(tiff, static_cast<uint16_t>(z));

        // Verify this page has same dimensions
        uint32_t pageW = 0, pageH = 0;
        TIFFGetField(tiff, TIFFTAG_IMAGEWIDTH, &pageW);
        TIFFGetField(tiff, TIFFTAG_IMAGELENGTH, &pageH);
        if (pageW != width || pageH != height) {
            TIFFClose(tiff);
            throw std::runtime_error("TIFF page " + std::to_string(z) +
                                     " has different dimensions: " + path.string());
        }
        ...
        if (TIFFIsTiled(tiff)) {
            ...
            std::vector<float> tileBuf(tileW * tileH);

            for (uint32_t y0 = 0; y0 < height; y0 += tileH) {
                for (uint32_t x0 = 0; x0 < width; x0 += tileW) {
                    ttile_t tileIdx = TIFFComputeTile(tiff, x0, y0, 0, 0);
                    if (TIFFReadEncodedTile(tiff, tileIdx, tileBuf.data(),
                                           static_cast<tmsize_t>(tileBuf.size() * sizeof(float))) < 0) {
                        TIFFClose(tiff);
                        throw std::runtime_error("Failed to read tile at (" +
                                                 std::to_string(x0) + "," + std::to_string(y0) +
                                                 ") page " + std::to_string(z) + ": " + path.string());
                    }

                    // Copy tile data into result array
                    uint32_t copyH = std::min(tileH, height - y0);
                    uint32_t copyW = std::min(tileW, width - x0);
                    for (uint32_t ty = 0; ty < copyH; ++ty) {
                        for (uint32_t tx = 0; tx < copyW; ++tx) {
                            result(z, y0 + ty, x0 + tx) = tileBuf[ty * tileW + tx];
                        }
                    }
                }
            }
            ...
            for (uint32_t y = 0; y < height; ++y) {
                if (TIFFReadScanline(tiff, scanline.data(), y) < 0) {
                    TIFFClose(tiff);
                    throw std::runtime_error("Failed to read scanline " + std::to_string(y) +
                                             " page " + std::to_string(z) + ": " + path.string());
                }
                for (uint32_t x = 0; x < width; ++x) {
                    result(z, y, x) = scanline[x];
                }
            }
        }
    }
