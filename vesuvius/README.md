...
| `vesuvius.audit_ct_support`   | `models.run.audit_ct_support`      | audits an existing prediction volume against the masked CT it was inferred from: exact voxel-level phantom fractions, or a zero-download chunk-level triage that reads only stored chunk keys |
