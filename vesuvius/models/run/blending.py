...
def merge_inference_outputs(
    ...
    if part_id == 0:
        ...
        while not zarr_array_exists(output_path) or not zarr_array_exists(weight_accumulator_path):
            if wait_time >= max_wait_time:
                raise RuntimeError(f"Timeout waiting for part 0 to create arrays. Waited {max_wait_time} seconds.")

            if verbose:
                print(f"  Part {part_id} waiting for arrays to be created by part 0... ({wait_time}s elapsed)")
            time.sleep(5)
            wait_time += 5
    ...
    # --- 10. Cleanup ---
    if delete_weights and num_parts == 1:
        # Only delete weights when not using partitioning
    ...
    elif delete_weights and num_parts > 1:
        print(f"Skipping weight deletion in partitioned mode. Delete manually after all parts complete: {weight_accumulator_path}")
