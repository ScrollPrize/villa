...
open_zarr grew chunk_cache_dir/chunk_cache_max_gb, but a setting nobody can reach
from predict is a setting nobody uses. These tests pin the forwarding at each
seam: predict's parser, VCDataset to Volume, and Volume to open_zarr.
...
@pytest.fixture
def chunk_cache_dir(tmp_path) -> str:
...
def _assert_forwarded(kwargs: dict, chunk_cache_dir: str) -> None:
    assert kwargs["cache"] is True
    assert kwargs["cache_size_mb"] == 8
    assert kwargs["chunk_cache_dir"] == chunk_cache_dir
    assert kwargs["chunk_cache_max_gb"] == 1.5


def _volume(zarr_path: str, chunk_cache_dir: str) -> Volume:
    return Volume(
        type="zarr",
        path=zarr_path,
        cache=True,
        ...
        chunk_cache_max_gb=1.5,
    )


def test_volume_init_forwards_cache_settings(zarr_path, chunk_cache_dir, monkeypatch):
    calls = _spy_open_zarr(monkeypatch, zarr_path)
    _volume(zarr_path, chunk_cache_dir)
    assert calls, "constructing a Volume opened no zarr store"
    for kwargs in calls:
        _assert_forwarded(kwargs, chunk_cache_dir)


def test_load_ome_metadata_forwards_cache_settings(zarr_path, chunk_cache_dir, monkeypatch):
    """The gap this task closes: metadata reads used to get no cache arguments."""
    calls = _spy_open_zarr(monkeypatch, zarr_path)
    vol = _volume(zarr_path, chunk_cache_dir)
    calls.clear()
...
def test_load_data_forwards_cache_settings(zarr_path, chunk_cache_dir, monkeypatch):
    calls = _spy_open_zarr(monkeypatch, zarr_path)
    vol = _volume(zarr_path, chunk_cache_dir)
    calls.clear()
    vol.load_data()
...
def test_volume_defaults_leave_the_disk_cache_off(zarr_path, monkeypatch):
    ...
    for kwargs in calls:
        ...
        assert kwargs["chunk_cache_dir"] is None
        assert kwargs["chunk_cache_max_gb"] is None


def test_vc_dataset_forwards_cache_settings_to_volume(zarr_path, chunk_cache_dir, monkeypatch):
    ...
    assert seen["chunk_cache_dir"] == chunk_cache_dir
    assert seen["chunk_cache_max_gb"] == 1.5
...
def test_vc_dataset_defaults_leave_the_cache_off(zarr_path, monkeypatch):
    ...
    assert seen["chunk_cache_dir"] is None
    assert seen["chunk_cache_max_gb"] is None
...
def test_predict_cli_accepts_the_cache_flags():
    ...
    args, _ = build_parser().parse_known_args(
        [
            ...
            "--output_dir", "o",
            "--chunk_cache_dir", "/x",
            "--chunk_cache_max_gb", "2",
        ]
    )
    assert args.chunk_cache_dir == "/x"
    assert args.chunk_cache_max_gb == 2.0
...
def test_predict_cli_cache_flags_default_to_none():
    ...
    assert args.chunk_cache_dir is None
    assert args.chunk_cache_max_gb is None
