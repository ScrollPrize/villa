...
def test_invert_affine_matrix_uses_block_inverse_for_scale_shear_translation() -> None:
    m = np.eye(4, dtype=np.float64)
    m[:3, :3] = np.asarray(
        [
            [2.0, 0.25, -0.1],
    ...
    got = affine.invert_affine_matrix(m)
    inv_linear = np.linalg.solve(m[:3, :3], np.eye(3))
    expected = np.eye(4, dtype=np.float64)
    expected[:3, :3] = inv_linear
    expected[:3, 3] = -inv_linear @ m[:3, 3]

    np.testing.assert_allclose(got, expected, atol=1e-12)
    np.testing.assert_allclose(m @ got, np.eye(4), atol=1e-12)


def test_invert_affine_matrix_rejects_projective_matrix() -> None:
    m = np.eye(4, dtype=np.float64)
    m[3, 0] = 0.1
    with pytest.raises(ValueError, match="homogeneous affine"):
        affine.invert_affine_matrix(m)
