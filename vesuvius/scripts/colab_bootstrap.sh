...
# Runs a Python snippet against the session via stdin.
remote_py() {
    local secs out rc=0
    secs="${2:-30}"
...
if [[ -n "$CACHED_WHEEL" ]]; then
    ...
    remote_bash "export PATH=\"\$HOME/.local/bin:\$PATH\"; cd '$REMOTE_HOME/villa/vesuvius' && uv pip install --python .venv/bin/python --force-reinstall '$CACHED_WHEEL'" 300
else
...
" 30 | tail -1)
    ...
    remote_bash "export PATH=\"\$HOME/.local/bin:\$PATH\"; cd '$REMOTE_HOME/villa/vesuvius' && uv pip install --python .venv/bin/python --force-reinstall '$BUILT_WHEEL'" 300
fi
