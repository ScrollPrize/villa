...
def create_optimizer(optimizer_config, model):
    ...
    if optim_name == 'adadelta':
        ...
        optimizer = optim.Adadelta(params, lr=learning_rate, rho=rho,
                                   weight_decay=weight_decay)
    elif optim_name == 'adagrad':
        lr_decay = optimizer_config.get('lr_decay', 0)
        optimizer = optim.Adagrad(params, lr=learning_rate, lr_decay=lr_decay,
                                  weight_decay=weight_decay)
        ...
        optimizer = optim.AdamW(params, **adamw_kwargs)
    elif optim_name == 'sparseadam':
        betas = tuple(optimizer_config.get('betas', (0.9, 0.999)))
        optimizer = optim.SparseAdam(params, lr=learning_rate, betas=betas)
    elif optim_name == 'adamax':
        betas = tuple(optimizer_config.get('betas', (0.9, 0.999)))
        adamax_kwargs = {'lr': learning_rate, 'betas': betas, 'weight_decay': weight_decay}
        if eps is not None:
            adamax_kwargs['eps'] = eps
        optimizer = optim.Adamax(params, **adamax_kwargs)
        ...
        optimizer = optim.Adamax(params, lr=learning_rate, lambd=lambd,
        ...
        optimizer = optim.LBFGS(params, lr=learning_rate, max_iter=max_iter,
        ...
        optimizer = optim.NAdam(params, lr=learning_rate, betas=betas,
        ...
        optimizer = optim.RAdam(params, **radam_kwargs)
    elif optim_name == 'rmsprop':
        alpha = optimizer_config.get('alpha', 0.99)
        optimizer = optim.RMSprop(params, lr=learning_rate, alpha=alpha,
                                  weight_decay=weight_decay)
    elif optim_name == 'rprop':
        momentum = optimizer_config.get('momentum', 0)
        optimizer = optim.RMSprop(params, lr=learning_rate, weight_decay=weight_decay, momentum=momentum)
    elif optim_name == 'sgd':
        ...
        optimizer = StableAdamW(params, weight_decay=weight_decay)
    
    elif optim_name == 'adabelief' : 
        from pytorch_optimizer import AdaBelief
        optimizer = AdaBelief(params, weight_decay=weight_decay)

    elif optim_name == 'muon' : 
        from pytorch_optimizer import Muon
        optimizer = Muon(params, weight_decay=weight_decay)
    
    elif optim_name == 'shampoo' :
        from pytorch_optimizer import Shampoo
        optimizer = Shampoo(params, weight_decay=weight_decay)
        ...
        if eps is not None:
            adam_kwargs['eps'] = eps
        optimizer = optim.Adam(params, **adam_kwargs)
