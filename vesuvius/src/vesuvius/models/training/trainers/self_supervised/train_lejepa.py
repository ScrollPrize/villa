...
class TrainLeJEPA(BaseTrainer):
    ...
    def _save_lejepa_debug(self, save_path: str, epoch: int):
        ...
        # Row 1: Original + Global views + Local views
        n_cols_row1 = 1 + n_global + min(n_local, 4)  # Cap local views shown

        # Original input
        ax = fig.add_subplot(3, n_cols_row1, 1)
        ...
        for i, gv in enumerate(global_views):
            ax = fig.add_subplot(3, n_cols_row1, 2 + i)
        ...
        for i, lv in enumerate(local_views[:4]):
            ax = fig.add_subplot(3, n_cols_row1, 2 + n_global + i)
            img = get_center_slice(lv)
            ax.imshow(img, cmap='gray')
            ax.set_title(f'Local {i+1}', fontsize=10)
            ax.axis('off')

        # Row 2: PCA of embeddings + Embedding histogram
        ...
        plt.suptitle(f'LeJEPA Debug - Epoch {epoch+1}', fontsize=14, fontweight='bold')
        plt.tight_layout()
