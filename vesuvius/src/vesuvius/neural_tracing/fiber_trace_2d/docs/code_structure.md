...
- `strip_geometry.py` ports the VC3D/Lasagna `LineViewBuilder` side-strip frame construction and builds explicit side-strip coordinate grids.
- `augmentation.py` implements deterministic strip augmentations using torch tensor operations for post-Zarr image/value changes.
- `loader.py` parses Vesuvius-style JSON configs, opens base-volume Zarr arrays through the existing Vesuvius cache-aware opener, samples explicit coordinates, and computes prefetch chunk requests.
