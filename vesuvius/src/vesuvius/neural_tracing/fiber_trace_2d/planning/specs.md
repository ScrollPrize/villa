...
- Direction-field inspection draws only every fourth source pixel in x and y, so each drawn sample corresponds to an 8x8 display-pixel cell in the 2x visualization. It draws anti-aliased 6-display-pixel direction segments, skips invalid image pixels and invalid/non-finite decoded directions, writes `dir_vis.jpg`, and writes sample/checkpoint/drawn-count metadata to `dir_vis_summary.txt`.
