...
- On CUDA training runs, `training.pipeline_enabled` may overlap future batch loading with current-batch model work through a bounded deterministic producer/consumer queue. `training.pipeline_depth` controls the number of submitted whole-batch futures; whole batches are still consumed strictly by step number, so the deterministic CP sample stream is unchanged.
...
- The pipeline uses the shared `FiberStrip2DLoader.load_batch` path with image/value augmentation deferred. Loaded batches carry the deterministic per-patch augmentation parameters, and the main training path applies the existing torch value augmentation immediately before normalization and forward pass.
- Whole-batch prefetch uses a single producer because each `load_batch` already uses `loader_workers` internally and cache tracing is scoped per `load_batch` call. This avoids concurrent cache-trace accounting races while overlapping one future batch with the current training step.
...
- Training config keys include `max_sample_index` for bounded deterministic-prefix reuse, `pipeline_enabled` and `pipeline_depth` for CUDA training load/model overlap, and `test_interval`, `test_control_points`, and `test_start_sample_index` for deterministic test evaluation when `test_datasets` is configured.
