...
- CUDA training prepares loaded batches on a separate CUDA stream when `training.pipeline_enabled` is true. Preparation applies deferred torch image/value augmentation, normalizes images, builds direction-supervision tensors, and records a CUDA event. The main training stream waits for that event immediately before forward pass.
