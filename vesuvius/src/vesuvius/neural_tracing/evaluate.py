...
@click.option("--checkpoint_path", type=click.Path(exists=True), required=True, help="Path to checkpoint file or directory of checkpoints passed through to trace")
...
def main(points_path, checkpoint_path, steps_per_crop, max_size):
    """
    ...
    for vol_idx, volume in enumerate(volumes):
        ...
        for pt_idx, point in enumerate(volume_points):
            ...
            uuid = f"nt-eval_{eval_timestamp}_{x}-{y}-{z}"
            click.echo(
