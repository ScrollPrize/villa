"""Train a step scorer for the volume-cartographer fiber beam search.
...
STATE_KEYS = ('n_history', 'k_back', 'pool_size', 'tolerance', 'progress_slack',
              'lateral_sigmas', 'lateral_probs', 'angle_sigmas_deg', 'angle_probs')
...
def build_parser():
    ...
    ap.add_argument('--ct-grid-scale', type=float, default=8., help='Base voxels per CT voxel; s1_ds2 level 1 uses 8')
    ...
    ap.add_argument('--pool-size', type=int, default=256, help='Uniform training proposal sample; inference scores all')
    ap.add_argument('--k-back', type=int, default=32)
    ap.add_argument('--lookahead-width', type=int, default=32)
    ...
    ap.add_argument('--compile', action=argparse.BooleanOptionalAction, default=True,
                    help='torch.compile the CUDA training forward (diagnostics stay eager)')
    ...
    ap.add_argument('--crop-spacing', type=float, default=1.)
    ...
    ap.add_argument('--norm', choices=('group',), default='group')
    ...
    ap.add_argument('--mine-threads', type=int, default=0,
                    help='C++ threads while mining hard spans in-process (0 = OpenMP); unused with --mine-workers > 1')
    ap.add_argument('--mine-workers', type=int, default=16,
                    help='Processes mining hard spans concurrently, one fiber each (<= 1 mines in-process)')
...
def main(argv=None):
    ...
    if min(args.crop_spacing, args.tolerance, args.history_sigma) <= 0:
        ap.error('Spacings, tolerance and sigmas must be positive')
    if args.crop_behind*args.crop_spacing < args.k_back or args.crop_depth <= args.crop_behind+1:
        ap.error('crop must cover candidate history and forward proposals')
    if args.diag_fibers < 0 or not math.isfinite(args.diag_max_len) or args.diag_max_len <= 0:
        ap.error('diag-fibers must be nonnegative and diag-max-len finite and positive')
    if not 0 <= args.diag_confidence <= 1:
        ap.error('diag-confidence must be between zero and one')
    ...
    state_cfg = BeamStateConfig(crop=crop, n_history=args.n_history, k_back=args.k_back,
                                pool_size=args.pool_size, tolerance=args.tolerance, lateral_sigmas=tuple(args.lateral_sigmas),
    ...
    if args.hard_spans != 'none':
        ...
        hard_spans = load_or_mine_hard_spans(cache, miner, train_f, spec.grid_scale,
                                             progress=lambda e: print(format_training_log(dict(mining=e)), flush=True),
                                             workers=args.mine_workers)
        print(format_training_log(dict(hard_spans=sum(map(len, hard_spans.values())), hard_span_fibers=len(hard_spans),
    ...
    dl = torch.utils.data.DataLoader(ds, pin_memory=cuda, **kwargs)
    it = iter(dl)
    opt = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=1e-4, fused=cuda)
    # The compiled wrapper shares the module's parameters; the diagnostic hook,
    # gradient clipping and checkpoints keep using ``model``.
    trainable = torch.compile(model) if args.compile and cuda else model

    diag = None
    if args.diag_every:
        from vesuvius.neural_tracing.fiber_follow.beam.diag import plot_pool, span_diag, rollout_images
    ...
    print(format_training_log(dict(train_fibers=len(train_f), val_fibers=len(val_f),
    ...
    try:
        for step in range(1, args.steps + 1):
            ...
            batch = {k: (torch.cat([p[k] for p in parts]) if len(parts) > 1 else parts[0][k])
                         .to(args.device, non_blocking=True) for k in parts[0]}
            ...
            with torch.autocast('cuda', dtype=torch.bfloat16, enabled=cuda):
                output = trainable(batch['x'].float(), batch['hist'], batch['hmask'], batch['candidates'],
                                   batch['point_mask'])
            loss, metrics = beam_loss(output, batch, args.rank_weight, args.onfiber_weight)
            ...
            if step % args.log_every == 0 or step == args.steps:
                log.record(dict(step=step, loss=loss.item(), lr=lr,
                                ...
                                samples_per_second=step * args.batch / (time.monotonic() - start),
            ...
            if diag and step % args.diag_every == 0:
                ...
                reports = diag['rollout_images'](diag['beam'], diag['hook'], diag['vol'],
                    diag['fibers'], diag['seeds'], out / 'images', step,
                    max_len=args.diag_max_len, confidence=args.diag_confidence)
                for report in reports:
                    log.record(dict(step=step, **report))
                if not reports:
                    log.record(dict(step=step, beam_rollout_skipped='no eligible held-out seeds'))
