...
def invert_affine_matrix(matrix: np.ndarray) -> np.ndarray:
    ...
    This uses the affine block inverse explicitly:

    ``[A t; 0 1]^-1 == [A^-1, -A^-1 t; 0 1]``

    Keeping this separate from a generic homogeneous matrix inverse makes the
    voxel-coordinate convention explicit and rejects accidental projective
    matrices before they can be used as registration transforms.
    """
    ...
    if not np.allclose(m[3], np.asarray([0.0, 0.0, 0.0, 1.0], dtype=np.float64), atol=1e-12):
        raise ValueError("matrix must be a homogeneous affine transform with last row [0, 0, 0, 1]")

    linear = m[:3, :3]
    translation = m[:3, 3]
    inv_linear = np.linalg.solve(linear, np.eye(3, dtype=np.float64))
    inv = np.eye(4, dtype=np.float64)
    inv[:3, :3] = inv_linear
    inv[:3, 3] = -inv_linear @ translation
    return inv
