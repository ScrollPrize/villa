...
from ink.recipes.data.masks import SUPERVISION_MASK_NAME, default_mask_name_for_split, resolve_segment_mask_names
...
class ZarrPatchDataRecipe:
    ...
    def build(self, *, runtime=None, augment=None) -> DataBundle:
        ...
        valid_context = contexts["valid"]
        train_segment_ids = split_segment_ids["train"]
        valid_segment_ids = split_segment_ids["valid"]

        train_dataset = ZarrPatchDataset(
            samples_by_split["train"],
            layout=train_context.layout,
            segments=train_context.segments,
            split="train",
            ...
            label_suffix=train_context.label_suffix,
            mask_suffix=train_context.mask_suffix,
            mask_name=train_context.mask_name,
            train_segment_ids=train_segment_ids,
            volume_cache=train_context.volume_cache,
            label_mask_store_cache=train_context.label_mask_store_cache,
            patch_index_cache_dir=train_context.patch_index_cache_dir,
            cache_patches_in_memory=bool(self.cache_train_patches_in_memory),
            include_valid_mask=bool(self.include_train_valid_mask),
        )
        valid_dataset = ZarrPatchDataset(
            samples_by_split["valid"],
            layout=valid_context.layout,
            segments=valid_context.segments,
            split="valid",
            ...
            stride=stride,
            in_channels=in_channels,
            label_suffix=valid_context.label_suffix,
            mask_suffix=valid_context.mask_suffix,
            mask_name=valid_context.mask_name,
            train_segment_ids=train_segment_ids,
            volume_cache=valid_context.volume_cache,
            label_mask_store_cache=valid_context.label_mask_store_cache,
            patch_index_cache_dir=valid_context.patch_index_cache_dir,
        ...
        log_data_progress(
            "[data] caches "
            f"volume_entries={len(contexts['train'].volume_cache)} "
            f"label_mask_store_entries={len(contexts['train'].label_mask_store_cache)} "
...
def build_zarr_split_samples(
    ...
    *,
    ...
    def _build_one(segment_id: str):
        ...
        if group_idx_by_segment is None:
            patch_rows = [
                (
                    segment_id,
                    tuple(int(value) for value in xyxy),
            ...
            segment_group_idx = int(group_idx_by_segment[segment_id])
            patch_rows = [
                (
                    segment_id,
                    tuple(int(value) for value in xyxy),
