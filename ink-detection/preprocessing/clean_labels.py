...
def parse_version_id(value: str) -> int:
    ...
    if normalized in {"base", "unversioned", "v1"}:
        return 1
    if normalized.startswith("v") and normalized[1:].isdigit():
        version_num = int(normalized[1:])
        if version_num >= 1:
            return version_num
    raise argparse.ArgumentTypeError(
        "--version-id must be one of 'base', 'unversioned', or 'vN' (for example 'v2')"
    )
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional label version selector. Use 'base' for unversioned files or "
            "'vN' such as 'v2' to process only that version."
        ),
    )
...
def is_target_image(path: Path, *, version_id: int | None = None) -> bool:
    parsed = parse_target_image(path)
    if parsed is None:
        return False
    if version_id is None:
        return True
    return int(parsed["version_num"]) == int(version_id)


def find_target_images(root: Path, *, version_id: int | None = None) -> List[Path]:
    matches: List[Path] = []
    for current_root, dirnames, filenames in os.walk(root):
        ...
        for filename in sorted(filenames):
            ...
            parsed = parse_target_image(candidate)
            if parsed is None:
                continue
            if version_id is not None and int(parsed["version_num"]) != int(version_id):
                continue

            stem_key = (
                f"{str(parsed['prefix']).lower()}::{str(parsed['label_kind']).lower()}"
                f"::v{int(parsed['version_num'])}"
            )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    image_paths = find_target_images(scan_root, version_id=args.version_id)
    if not image_paths:
