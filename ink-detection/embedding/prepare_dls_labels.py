...
NONZERO_PADDING = 16
FRAGMENT_DOWNSAMPLING_FACTOR = 4  # fragments are ~4um
DEFAULT_DOWNSAMPLING_FACTOR = 2  # scrolls are ~8um
...
def downsampling_factor_for(source_root: Path, image_path: Path) -> int:
    relative_path = image_path.relative_to(source_root)
    top_level_name = relative_path.parts[0]
    if top_level_name.startswith("Fragment"):
        return FRAGMENT_DOWNSAMPLING_FACTOR
    assert top_level_name.startswith("Scroll")
    return DEFAULT_DOWNSAMPLING_FACTOR
...
def main(source: Path, dest: Path) -> None:
    """Downsample DLS label images from SOURCE and write flattened PNGs into DEST."""
    ...
    for image_path in image_paths:
        ...
        downsampling_factor = downsampling_factor_for(source, image_path)
        try:
