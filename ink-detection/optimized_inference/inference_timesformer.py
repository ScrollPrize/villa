...
def convert_to_human_readable(mask_pred: np.ndarray, 
                            output_format: str = "uint8") -> np.ndarray:
    """
    Convert prediction mask to human-readable format.
    
    Args:
        mask_pred: Prediction mask with values in [0, 1]
        output_format: Output format ("uint8" for 0-255 range, "float32" for 0-1 range)
